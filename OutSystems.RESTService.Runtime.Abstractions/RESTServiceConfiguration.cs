...
namespace OutSystems.RESTService.Runtime.Abstractions {
    public class RESTServiceConfiguration {
        ...
        public bool Trace => TraceErrors || TraceAll;
        ...
        private static class ConfigurationCache {
            ...
            public static RESTServiceConfiguration GetESpaceCachedValue(string key, string cacheName, int eSpaceId, Func<string, RESTServiceConfiguration> Getter) {
                ...
                return InnerGetCachedValue(cacheKey, Getter, eSpaceId);
            }

            private static RESTServiceConfiguration InnerGetCachedValue(string key, Func<string, RESTServiceConfiguration> Getter, int cacheExtraId) {
                lock (lockObject) {
                    if (reentrantCall) {
                    ...
                    reentrantCall = true;
                    try {
                        var configuration = RuntimeCache.Instance.GetOrAdd(new CacheKey(key),
                                                                            () => Getter(key),
                                                                            new EspaceTenantDependency(cacheExtraId, 0),
                                                                            DateTime.UtcNow.AddDays(1),
                                                                            TimeSpan.Zero,
                                                                            CacheItemPriority.Removable);
                        return (RESTServiceConfiguration)configuration;
                    } finally {
                        reentrantCall = false;
                    }
                }
