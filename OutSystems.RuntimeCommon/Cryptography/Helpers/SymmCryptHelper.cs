...
namespace OutSystems.RuntimeCommon.Cryptography {
    ...
    public static class SymmCryptHelper {
        ...
        /// </summary>
        /// <param name="outputStream">The output stream.</param>
        /// <param name="key">The password used to derive the key</param>
        ...
        public static string EncryptWithAES256FixedIV(string password, string content) {
            return EncryptWithAES256FixedIV(password, content, PasswordHelper.RFC_2898_DERIVE_BYTES_SALT);
        }

        public static string DecryptWithAES256FixedIV(string password, string cipherText) {
            return DecryptWithAES256FixedIV(password, cipherText, PasswordHelper.RFC_2898_DERIVE_BYTES_SALT);
        }
        ...
        public static byte[] DecryptThenMacWithAESDotNet(byte[] keyBytes, byte[] allBytes, byte[] associatedData) {
            using (Aes crypto = new AesManaged() {
                ...
                if (allBytes.Length - iv.Length - macBytes.Length < 0) {
                    throw new InvalidOperationException("Cannot decrypt content");
                }
