...
namespace OutSystems.RuntimeCommon.Extensibility.Plugins {
    ...
    public abstract class AbstractPluginProvider<KeyType, PluginType>: IPluginProvider {
        ...
        /// <param name="isRuntimePlugin">true if the plugin is to be used in applications runtime. false to only lookup /plugin/* folders.</param>
        ...
        protected virtual IEnumerable<PluginType> DiscoverTypesInAssembly(Assembly assembly, PluginProviderLogErrorDelegate logError) {
            var discoveredTypes = new List<PluginType>();
            IEnumerable<Type> allTypes;
            try {
                allTypes = assembly.GetTypes();
            } catch (ReflectionTypeLoadException rtle) {
                allTypes = rtle.Types.Where(t => t != null);
                rtle.LoaderExceptions.Apply(e => logError(e, "Error loading types from plugin:" + assembly.GetName()));
            }
        ...
        public virtual void LoadPlugins(PluginProviderLogErrorDelegate logError) {
            ...
            foreach (var asm in toScan) {
                try {
                    var typesDiscovered = DiscoverTypesInAssembly(asm, logError);
