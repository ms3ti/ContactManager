    ...
    public static class StringUtils {
        ...
        public static bool IsSelectStatement(string sql) {
            if (validQueryForCountMultiLineRegex == null) {
                validQueryForCountMultiLineRegex = new Regex(validQueryForCountPattern, commonOptions | RegexOptions.Multiline);
            }
            return validQueryForCountMultiLineRegex.IsMatch(sql);
        }
        ...
        // this method is used as a quick fix for #RDEV-945
        public static string RemoveImportDetails(string description) {
            if (description == null) {
                return null;
            }
            var index = description.IndexOf("Import Details:");
            return index >= 0 ? description.Substring(0, index).Trim() : description;
        }

        // this method is used as a quick fix for #RDEV-945
        public static string TruncateDescription(string description) {
            const int descriptionMaxSize = 2000;
            if (description == null) {
                return null;
            }
            return description.TrimWithEllipsis(descriptionMaxSize - 3);
        }
    }
}
