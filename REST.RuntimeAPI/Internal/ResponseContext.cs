        ...
        private void EnsureBinaryIsInitialized() {
            ...
            if (!httpResponseAlreadyRead) {
                try {
                    ...
                    using (MemoryStream responseMemoryStreamFix = new MemoryStream()) {
                        using (var responseStream = httpResponse.GetResponseStream()) {
                            int readBytes;
                            while ((readBytes = responseStream.Read(responseBuffer, 0, responseBuffer.Length)) > 0) {
                                responseMemoryStreamFix.Write(responseBuffer, 0, readBytes);
                            }
                        }
                        responseBodyAsBinary = responseMemoryStreamFix.ToArray();
                    }
                  
                } catch (IOException) {
                    responseBodyAsBinary = Array.Empty<byte>();
                }
