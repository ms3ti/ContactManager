...
namespace OutSystems.HubEdition.Extensibility.Data.TransactionService {
    ...
    public class GenericTransactionManager : ITransactionManager {
        ...
        protected TransactionInfo GetTransactionInfo(IDbTransaction trans) {
            return GetTransactionInfo(trans, out bool isRequestTransaction);
        }
        ...
        public virtual IDbTransaction GetRequestTransaction() {
            lock (this) {
                if (RequestTransactionInfo == null) {
                    ...
                    OSTrace.Info("DBConnectionPooling[TID=" + RequestTransactionInfo.Transaction.GetHashCode() + "] - reserved from pool");
...
#if DEBUG_DBCONNECTIONPOOLING
                        OSTrace.Info("DBConnectionPooling[TID=" + RequestTransactionInfo.Transaction.GetHashCode() + "] - returned to pool");
                        OSTrace.Error(new StackTrace().ToString());
#endif
...
#if DEBUG_DBCONNECTIONPOOLING
                    OSTrace.Debug("DBConnectionPooling[TID=" + transInfo.Transaction.GetHashCode() + "] - releasing poolable transaction");
#endif
                    transInfo.Release();
                } else {
#if DEBUG_DBCONNECTIONPOOLING
                    OSTrace.Debug("DBConnectionPooling[TID=" + transInfo.Transaction.GetHashCode() + "] - releasing non-poolable transaction");
#endif
            ...
            if (RequestTransactionInfo != null && RequestTransactionInfo.Transaction.Equals(trans)) {
                ...
                if (trans != null) {
                    ...
                    if (transInfo != null) {
                        ...
                        SafeEndTransaction(transInfo, commit, /*toFreeResources*/false, out CloseConnectionTransactionException closeConnectionException);
                        if (CheckConnectionCloseException(closeConnectionException)) {
                            throw closeConnectionException;
                        }
                    }
        ...
        protected virtual void EndTransaction(TransactionInfo transInfo, bool commit, bool toFreeResources) {
            ...
            try {
                ...
                stopwatch.Stop();
                // #RSAT-2502: measure the time the commit or rollback took to fail. This can be important to troubleshoot
                // SQL Server Multi-AZ Execution timeouts 
                var timeOutErrorMessage = $"The {(commit ? "Commit" : "Rollback")} failed with {ieo.Message} after {stopwatch.ElapsedMilliseconds} ms.";
                OSTrace.Error(timeOutErrorMessage);
                EventLogger.WriteError(timeOutErrorMessage);
                throw;
            } finally {
                transInfo.ReturnConnectionToPool();
            }
        ...
        public void AbortTransaction(IDbTransaction trans) {
            ...
            TransactionInfo transInfo = GetTransactionInfo(trans, out bool isRequestTransaction);
