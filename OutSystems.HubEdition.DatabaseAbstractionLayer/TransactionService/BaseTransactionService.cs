    ...
    public abstract class BaseTransactionService: ITransactionService {
        ...
        public virtual bool TestConnection(out string errorMessage) {
            ...
            try {
                ...
                // If DBProvider plugin has an instance of DatabasePropertiesService, use it to get message.
                (DatabaseServices.ExecutionService as BaseExecutionService)?.DatabasePropertiesService?.CheckIfVersionIsSupported(out errorMessage, conn);
            } catch (Exception e) {
                errorMessage = e.Message;

                EventLogger.WriteError($"Error testing connection to the database: {e.Message}\n\n{e.StackTrace}\n{new StackTrace(true)}");

                // Failed connections "poison" the connection pool for a few seconds, clearing it explicitly 
