...
namespace OutSystems.HubEdition.Extensibility.Data.ConfigurationService {
    ...
    public static class SerializationConstants {
        public const string TAG_KEY = "DBConfigurationTag";
    }
    ...
    public sealed class Serializer<TConfiguration> {
        ...
        private Func<TConfiguration, IEnumerable<IPluginParameter>> ParameterExtractor { get; }
    ...
    public sealed class DictionarySerializer<TConfiguration> {
        ...
        internal DictionarySerializer(string tag, Func<TConfiguration, IEnumerable<IPluginParameter>> parameterExtractor) {
            this.Tag = tag;
            this.ParameterExtractor = parameterExtractor;
            this.ValueEncrypter = null;
        }

        public Dictionary<string, string> Serialize(TConfiguration conf) {
            var res = new Dictionary<string, string> {{SerializationConstants.TAG_KEY, Tag}};

            ParameterExtractor(conf)
                .Apply(p => {
                    res.Add(p.Name, p.Get());
                });
            return res;
        }
    }
    ...
    public sealed class Deserializer<TConfiguration> {
    ...
    public sealed class DictionaryDeserializer<TConfiguration> {

        public string Tag { get; }
        public Func<string, string> ValueDecrypter { get; set; }

        internal DictionaryDeserializer(string tag) {
            Tag = tag;
            ValueDecrypter = null;
        }

        public void Deserialize(Dictionary<string, string> confDictionary, TConfiguration conf) {

            if (confDictionary == null) {
                throw new ConfigurationSerializationException("Dictionary cannot be null");
            }

            if (!confDictionary.TryGetValue(SerializationConstants.TAG_KEY, out var tagName) || tagName != Tag) {
                throw new ConfigurationSerializationException("Expected key tagged " + Tag + " got " + tagName);
            }

            var meta = new MetaDatabaseConfiguration(conf);

            foreach (var elem in confDictionary) {
                var param = meta.GetParameter(elem.Key);
