...
namespace OutSystems.Application.Scheduling {
    /// <summary>
    ...
    */
    ...
    public class RuntimeScheduler {
        ...
        public static bool ValidateSchedule(string schedule) {
            ...
            ValidateAndSeparateScheduleParts(scheduleParts, out schType, out List<string> hoursInSchedule, out List<string> daysInSchedule, out List<string> weekdaysInSchedule, out string ordinalInSchedule);
        ...
        private static void CalculateNextDayOfMonthSpan(
            ...
            foreach (string day in dayOccurences) {
                foreach (string time in timeOccurences) {
                    nextTs = new TimeSpan(int.Parse(day), 0, 0, 0).Add(TimeSpan.Parse(time + ":00"));
                    if (firstTime) {
                        firstTs = nextTs;
                        firstTime = false;
                    }
        ...
        private static void CalculateNextWeekdayOfMonthSpan(
            ...
            if (dt.Day > current.Day) {
                // future date, add first time occurrence
            ...
            foreach (string elm in ocurrences) {
                switch (type) {
                    case listType.MonthDay:
                        if (!sl.ContainsKey(int.Parse(elm)) && elm != string.Empty) {
                            sl.Add(int.Parse(elm), elm);
                        }
                        break;
                    case listType.Time:
