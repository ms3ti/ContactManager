    ...
    public abstract class AbstractContentCallback : AbstractCallback {
        ...
        public sealed override string RetrieveContentInjection(AppInfo app, SessionInfo session, CallbackPageContext context) {
            if (app.Context.Request.UserAgent != null &&
                ...
                IsADisabledTaskBox(app)) {
                // disable content injection for RichMail requests and when taskbox is disabled (if this is a taskbox callback)
                return string.Empty;
            } else {
        ...
        private bool IsADisabledTaskBox(AppInfo app) =>
            ProviderName == CallbackProviders.EPA_Taskbox && app.OsContext.ResponseDisabledTaskBox;

        private string PostProcess(string content, CallbackPageContext context) {
            if (String.IsNullOrEmpty(content)) {
                return string.Empty; // Don't let the processing continue with null
            }
            return content.
