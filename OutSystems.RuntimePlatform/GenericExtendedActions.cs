...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public class GenericExtendedActions {
        ...
        [Obsolete("Please use the other overload. This method will now have a performance impact.")]
        public static bool Check(HeContext heContext, ObjectKey key, int userId, int eSpaceId) {
            ObjectKey roleOwnerEspaceKey;
            using (var tran = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                roleOwnerEspaceKey = DBRuntimePlatform.Instance.GetESpaceKeyById(tran, eSpaceId);
            }

            return Check(heContext, key, roleOwnerEspaceKey, userId);
        }

        public static bool Check(HeContext heContext, ObjectKey roleKey, ObjectKey roleOwnerModuleKey, int userId) {
            bool hasRole;
            ...
            if (userId == heContext.Session.UserId) {
                return heContext.Session.CheckRole(
                    heContext.GetRoleIdFromKey(
                        roleKey.ToString(ObjectKey.DatabaseFormat),
                        roleOwnerModuleKey.ToString(ObjectKey.DatabaseFormat)));
            } else {

                // Authorization (Roles mapping) is defined on SystemDB
                using var tran = RuntimePlatformSettings.Misc.ReadUncommittedOrReadCommittedWithMARS.GetValue() ? DatabaseAccess.ForSystemDatabase.GetRequestTransaction() :
                                                                                         DatabaseAccess.ForSystemDatabase.GetReadOnlyTransaction();
                ...
                hasRole = DBRuntimePlatform.Instance.CheckRole(tran, userId, roleOwnerModuleKey, roleKey, tenantId);
        ...
        public static void Grant(HeContext heContext, ObjectKey key, bool isPersistent, int userId, int eSpaceId) {
            ...
            Grant(heContext, key, roleOwnerEspaceKey, isPersistent, userId);
        }

        public static void Grant(HeContext heContext, ObjectKey roleKey, ObjectKey roleOwnerModuleKey, bool isPersistent, int userId) {
            int tenantId = heContext.Session.TenantId;
            ...
            if (userId == heContext.Session.UserId) {
                ...
                var globalKey = new GlobalObjectKey(roleOwnerModuleKey, roleKey);
                heContext.Session.AddRole(
                    heContext.GetRoleIdFromKey(
                        roleKey.ToString(ObjectKey.DatabaseFormat),
                        roleOwnerModuleKey.ToString(ObjectKey.DatabaseFormat)),
                    globalKey.ToString(ObjectKey.ShortFormat));
            }
            ...
            if (isPersistent) {
                ...
                if (userId != 0) {
                    // RoleId must be defined on App Runtime DB
                    using (var tran = RuntimePlatformSettings.Misc.ReadUncommittedOrReadCommittedWithMARS.GetValue() ? DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction() :
                                                                                              DatabaseAccess.ForRuntimeDatabase.GetReadOnlyTransaction()) {
                        roleId = Convert.ToInt32(DBRuntimePlatform.Instance.GetRoleId(tran, roleKey, roleOwnerModuleKey));
                    }

                    // Authorization info (Roles mapping) is defined on SystemDB
        ...
        public static void Revoke(HeContext heContext, ObjectKey key, bool isPersistent, int userId, int eSpaceId) {
            ...
            Revoke(heContext, key, roleOwnerEspaceKey, isPersistent, userId);
        }

        public static void Revoke(HeContext heContext, ObjectKey roleKey, ObjectKey roleOwnerModuleKey, bool isPersistent, int userId) {
            int tenantId = heContext.Session.TenantId;
            ...
            if (isPersistent && userId != 0) {
                // Authorization (Roles mapping and User) is defined on SystemDB
                using (var tran = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
                    // Check userID exists in DB
                    if (Convert.ToInt32(DBRuntimePlatform.Instance.GetNumUsersWithId(tran, userId)) != 1) {
                        throw new InvalidOperationException("Invalid User Id");
                    }
                    DBRuntimePlatform.Instance.DeleteUserRole(tran, userId, roleOwnerModuleKey, roleKey, tenantId);
                }
            }
        }
            ...
            try {
                ...
                if (heContext != null) {
                    ...
                    if (appInfo != null) {
                        ...
                        GeneralLog log = RuntimePlatformUtils.GenerateEspaceInvalidateMessage(heContext.Session.SessionGuid, espaceId, tenantId, heContext.Session.UserId, appInfo.eSpaceId, appInfo.Tenant.Id, extraInfo);
                        log.Write();
                    }
                }
            } catch (Exception e) {
        ...
        public static void TenantSwitch(HeContext heContext, int tenantId) {
            string username = string.IsNullOrEmpty(heContext.Session.UserName) ? "Anonymous user" : $"User {heContext.Session.UserName}";
            string tenant = heContext.Session.TenantId == tenantId ? $"remained with tenant {tenantId}" : $"switched from tenant {heContext.Session.TenantId} to {tenantId}";

            if (!heContext.AllowTenantSwitch && heContext.Session.TenantId != tenantId) {
                OSTrace.Debug($"{username} with id {heContext.Session.UserId} with tenant {heContext.Session.TenantId} failed switching to {tenantId}. TenantSwitch operation is not supported in this context.");
                throw new InvalidOperationException("TenantSwitch operation is not supported in this context.");
            }
            OSTrace.Debug($"{username} with id {heContext.Session.UserId} {tenant}.");
        ...
        public static void ChangeSiteProperty(HeContext heContext, ObjectKey propKey, string name, object val) {
            ...
            using (Transaction tran = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                int propId = DBRuntimePlatform.Instance.GetSitePropertyId(tran, propKey, eSpaceId, out bool isShared);
...
#if !NETSTANDARD // No support for reading ClientCertificates in Asp.Net Core apps
        ...
        public static void SetCurrentLocale(HeContext heContext, string locale) {
            ...
            if (RuntimePlatformSettings.Debug.DebugSetCurrentLocale.GetValue()) {
                try {
                    GeneralLog.StaticWrite(DateTime.Now, heContext.Session.SessionGuid, heContext.AppInfo.eSpaceId,
...
#if !NETSTANDARD // Ajax Notify Widget actions only apply to Traditional Apps
        ...
        public static void Deprecated_NotifyWidget(HeContext heContext, string widgetId, string message) {
            ...
            heContext.QueueJavascript(Web.JavaScriptManager.osAjaxNotifyWidget(widgetId, message));
        }
        ...
        public static void ActivityOpen(HeContext heContext, int Id, out string handlingUrl) {
            InnerActivityOpen(heContext, Id, /*onlyGetUrl*/false, out bool success, out string failureMessage, out handlingUrl, kind => kind.CanBeOpened(), "ActivityOpen");
            if (!success) {
        ...
        public static void ActivityGetUrl(HeContext heContext, int Id, out string handlingUrl) {
            // TODO MSM: Check this
            InnerActivityOpen(heContext, Id, /*onlyGetUrl*/true, out bool success, out string failureMessage, out handlingUrl, kind => kind.CanBeOpened(), "ActivityGetUrl");
            if (!success) {
                ProcessBase.ThrowSpecificException(failureMessage);
            }
        }
        ...
        private static void InnerActivityOpen(HeContext heContext, int activityId, bool onlyGetUrl, out bool success, out string failureMessage, out string handlingUrl, Func<ActivityKind, bool> activityKindCheck, string actionName) {
            ...
            BPMRuntime.GetActivityDataForWS(activityId, out int processId, out ActivityKind activityKind, out string activityName, out string SSKey, out string url, out int tenantId, out string espaceSSKey);
            ...
            using (IActivitiesClient activityHandler = new ActivitiesClientFactory(url, tenantId, userId, heContext.AppInfo.eSpaceUID, espaceSSKey).CreateClient()) {
                //ToDo: activityHandler.Timeout = (int)(1.2 * TimeoutInSec) * 1000;
                success = activityHandler.ExecuteOnOpen(SSKey, activityId, processId, tenantId, userId, BuiltInFunction.GetCurrentLocale(), heContext.Session.SessionID, heContext.CurrentHttpContext.Request.IsSecureConnection, heContext.CurrentHttpContext.Request.Url.Host, onlyGetUrl, out failureMessage, out handlingUrl);
                handlingUrl = BuiltInFunction.AddPersonalAreaToURLPath(handlingUrl);
            }
            handlingUrl = RuntimePlatformUtils.CheckUriSecurity(HttpService.Context.Request, handlingUrl);
        }
        ...
        public static void ActivityReset(HeContext heContext, int activityId, bool resetActivityUser) {
            ...
            using (IActivitiesClient activityHandler = new ActivitiesClientFactory(url, tenantId, userId, heContext.AppInfo.eSpaceUID, espaceSSKey).CreateClient()) {
                //ToDo: activityHandler.Timeout = (int)(1.2 * TimeoutInSec) * 1000;
                success = activityHandler.ExecuteActivityReset(SSKey, activityId, processId, resetActivityUser, tenantId, userId, BuiltInFunction.GetCurrentLocale(), out string failureMessage);
                if (!success) {
                    ProcessBase.ThrowSpecificException(failureMessage);
                }
            }
        ...
        public static void ActivitySchedule(HeContext heContext, int activityId, DateTime startDate) {
            ...
            using (IActivitiesClient activityHandler = new ActivitiesClientFactory(url, tenantId, userId, heContext.AppInfo.eSpaceUID, espaceSSKey).CreateClient()) {
                //ToDo: activityHandler.Timeout = (int)(1.2 * TimeoutInSec) * 1000;
                success = activityHandler.ExecuteActivitySchedule(SSKey, activityId, processId, startDate, tenantId, userId, BuiltInFunction.GetCurrentLocale(), out string failureMessage);
                if (!success) {
                    ProcessBase.ThrowSpecificException(failureMessage);
                }
            }
        ...
        private static void ActivityPushFromCreated(HeContext heContext, int processId, string activityssKey, int activityId) {
            ...
            BPMRuntime.GetActivityDataForWS(activityId, out processId, out ActivityKind activityKind, out string activityName, out string ssKey, out string url, out int tenantId, out string espaceSSKey);

            using (IActivitiesClient activityHandler = new ActivitiesClientFactory(url, tenantId, heContext.Session.UserId, heContext.AppInfo.eSpaceUID, espaceSSKey).CreateClient()) {
                activityHandler.ExecuteActivityAction(activityssKey, activityId, processId, tenantId, ActivityStatus.Created, BuiltInFunction.NullDate());
            }
        }
        ...
        private static int InnerActivityClose(HeContext heContext, int activityId, bool advanceProcess, bool skip, bool skipAll, Action</*id*/int, /*name*/string, ActivityKind, /*key*/string> activityChecks, List<Pair<string, object>> inputs) {
            ...
            using (IActivitiesClient activityHandler = new ActivitiesClientFactory(url, tenantId, userId, heContext.AppInfo.eSpaceUID, espaceSSKey).CreateClient()) {
                bool success = activityHandler.ExecuteOnClose(SSKey, activityId, processId, tenantId, userId, BuiltInFunction.GetCurrentLocale(), advanceProcess, skip, skipAll, inputs, out string failureMessage, out nextActIds);
        ...
        public static int ProcessLaunch(HeContext heContext, ObjectKey SSKey, ObjectKey espaceSSKey, int parentActivityId, int parentProcessId, List<Pair<string, object>> inputs, out int nextHumanActivity) {
            ...
            int newProcessId = ProcessLaunch(heContext, SSKey, espaceSSKey, parentActivityId, parentProcessId, inputs, out List<Pair<int, ActivityKind>> nextActIds);
        ...
        public static int ProcessLaunch(HeContext heContext, ObjectKey SSKey, ObjectKey espaceSSKey, int parentActivityId, int parentProcessId, List<Pair<string, object>> inputs, out List<Pair<int, ActivityKind>> nextActIds) {
            ...
            using (IActivitiesClient activityHandler = new ActivitiesClientFactory(url, heContext.Session.TenantId, heContext.Session.UserId, heContext.AppInfo.eSpaceUID, ObjectKeyUtils.DatabaseValue(espaceSSKey)).CreateClient()) {
        ...
        public static void ProcessTerminate(HeContext heContext, int processId) {
            ...
            int userId = tenantId == heContext.Session.TenantId ? heContext.Session.UserId : 0;
            using (IActivitiesClient activityHandler = new ActivitiesClientFactory(url, tenantId, userId, heContext.AppInfo.eSpaceUID, consumerKey).CreateClient()) {
