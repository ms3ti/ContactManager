...
namespace OutSystems.HubEdition.RuntimePlatform {
    public class RequestEventsGenerator {
        ...
        public Func<bool> IsInternalCallExecutedEnabled = () => RuntimePlatformSettings.Misc.RequestTracerInternalCallExecuted.GetValue();
        public Func<bool> IsServiceActionExecutedEnabled = () => RuntimePlatformSettings.Misc.RequestTracerServiceActionExecuted.GetValue();
        public Func<bool> ScreenServerExecutedEnabled = () => RuntimePlatformSettings.Misc.RequestTracerScreenServerExecutedEnabled.GetValue();
        public Func<bool> TimerExecutedEnabled = () => RuntimePlatformSettings.Misc.RequestTracerTimerExecutedEnabled.GetValue();
        public Func<bool> TraceFromUndefinedMainEvents = () => RuntimePlatformSettings.Misc.RequestTracerUndefinedMainEventExecuted.GetValue();
        ...
        public IEnumerable<RequestEvent> CreateEvents() {
            try {
                ...
                if (!TraceFromUndefinedMainEvents() && tracer.MainEventType == RequestTracerEventType.Undefined) {
        ...
        public void CreateEvent(RequestTracerEventType eventType, DateTime startInstant, RequestEventDetails details) {
            try {
                if (!IsPerformanceMonitorEnabled() ||
                    ...
                    //we ignore ExposedServiceActionExecuted because the events that are under this mainevent are what matter. and they're bound to something else that is consuming the serviceaction
                    (eventType == RequestTracerEventType.ExposedServiceActionExecuted) || 
                    (eventType == RequestTracerEventType.ScreenServer && !ScreenServerExecutedEnabled()) ||
                    (eventType == RequestTracerEventType.TimerExecuted && !TimerExecutedEnabled())) {
                    return;
                }
