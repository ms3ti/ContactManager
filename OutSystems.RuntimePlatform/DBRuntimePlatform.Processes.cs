...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public partial class DBRuntimePlatform {
        ...
        public int GetPrecedentActivityId(Transaction tran, int currentActivity, string targetSSKey) {
            ...
            return cmd.ExecuteScalar<int?>() ?? BuiltInFunction.NullIdentifier();
        }
        ...
        public IDataReader GetActivityOutput(Transaction tran, ObjectKey outputSSKey, int activityId) {
            Command cmd = tran.CreateCommand(
                $@"SELECT OUTPUT_VALUE {tran.DatabaseServices.DMLService.Identifiers.EscapeIdentifier("VALUE")}, DATA_TYPE
        ...
        public IDataReader GetSubProcessOutput(Transaction tran, ObjectKey outputSSKey, int activityId) {
            Command cmd = tran.CreateCommand(
                $@"SELECT O.SS_KEY, O.OUTPUT_VALUE {tran.DatabaseServices.DMLService.Identifiers.EscapeIdentifier("VALUE")}, O.DATA_TYPE
        ...
        public IDataReader GetProcessInput(Transaction tran, ObjectKey inputSSKey, int processId) {
            Command cmd = tran.CreateCommand(
                $@"SELECT SS_KEY, INPUT_VALUE {tran.DatabaseServices.DMLService.Identifiers.EscapeIdentifier("VALUE")}, DATA_TYPE DATA_TYPE
        ...
        private void AddOrUpdateActivityVariable(Transaction tran, int id, int tenantId, IActivityVariable variable, bool isActivity, bool isInput) {
            ...
            int variableId = cmd.ExecuteScalar<int>();
        ...
        public IDataReader GetProcessDataForWS(Transaction tran, int processId) {
            ...
            //TODO: #RAT-95 - Uses of 1 and 0 as booleans should be refactored
            Command cmd = tran.CreateCommand(
                "SELECT E.NAME AS ESPACE_NAME, E.SS_KEY AS ESPACE_KEY, PD.SS_KEY, P.TENANT_ID AS TENANT_ID, PD.IS_ACTIVE as IS_ACTIVE  " +
                "FROM OSSYS_BPM_PROCESS P " +
        ...
        public class Role {
            ...
            public ObjectKey RoleKey { get; set; }
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        public IEnumerable<Role> GetActivityRoles(Transaction tran, int activityId) {
            //TODO: #RAT-95 - Uses of 1 and 0 as booleans should be refactored
            Command cmd = tran.CreateCommand(
                @"SELECT E.SS_KEY ESPACE_KEY, P.SS_KEY ROLE_KEY
                FROM OSSYS_BPM_ACTIVITY ACT
                ...
                INNER JOIN OSSYS_ESPACE E ON P.ESPACE_ID = E.ID
            ...
            using (var reader = cmd.ExecuteReader()) {
                while (reader.Read()) {
                    roles.Add(new Role() {
                        EspaceKey = ObjectKey.Parse(reader.SafeGet<string>("ESPACE_KEY")),
                        RoleKey = ObjectKey.Parse(reader.SafeGet<string>("ROLE_KEY"))
                    });
                }
                return roles;
            }
        ...
        public void UpdateHumanActivityParams(Transaction tran, int activityId, int? userId, int? groupId, DateTime? dueDate, DateTime? activationInstant, string inboxDetail) {
            ...
            if (groupId != null) {
                if (groupId == 0) {
                    cmd.CreateParameter("@GROUPID", DbType.Int32, DBNull.Value);
                } else {
                    cmd.CreateParameter("@GROUPID", DbType.Int32, groupId);
                }
            }
            if (dueDate != null) {
                cmd.CreateParameter("@DUEDATE", dueDate);
            }
            if (activationInstant != null) {
                if (activationInstant.Value == BuiltInFunction.NullDate()) {
                    cmd.CreateParameter("@NEXTRUN", DbType.DateTime, DBNull.Value);
                } else {
                    cmd.CreateParameter("@NEXTRUN", activationInstant.Value);
                }
            }
            if (inboxDetail != null) {
        ...
        public void UpdateActivityNextRun(Transaction tran, int activityId, DateTime nextRun) {
            ...
            if (nextRun == BuiltInFunction.NullDate()) {
                ...
                cmd.CreateParameter("@NEXTRUN", nextRun);
            }
            cmd.CreateParameter("@ACTIVITYID", DbType.Int32, activityId);
        ...
        public ActivityStatus GetActivityStatus(Transaction tran, int activityId) {
            ...
            return (ActivityStatus)cmd.ExecuteScalar<int>();
        }
        ...
        public ProcessStatus GetProcessStatus(Transaction tran, int processId) {
            ...
            return (ProcessStatus)cmd.ExecuteScalar<int>();
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        public void SetProcessStatus(Transaction tran, int tenantId, int processId, ProcessStatus status, int userId) {
            OSTrace.Debug($"[DBRuntimePlatform.Processes > SetProcessStatus] tenantId = {tenantId}; processId = {processId}; status = {status}; userId = {userId}");
            ...
            if (status == ProcessStatus.Unknown) {
                cmd.CreateParameter("@STATUSID", DbType.Int32, DBNull.Value);
            } else {
                cmd.CreateParameter("@STATUSID", DbType.Int32, (int)status);
            }
            cmd.CreateParameter("@LASTMODIFIED", BuiltInFunction.CurrDateTime());
            if (userId == 0) {
                cmd.CreateParameter("@LASTMODIFIEDBY", DbType.Int32, DBNull.Value);
            } else {
        ...
        public int CreateActivity(Transaction tran, int tenantId, int activityDefId, int processId, string name, int userId, int precedentActivityId, string precedentOutcome, DateTime? dueDate, string inboxDetail) {
            ...
            if (userId == 0) {
                sqlCmd.CreateParameter("@SSUSER_ID", DbType.Int32, DBNull.Value);
            } else {
                sqlCmd.CreateParameter("@SSUSER_ID", DbType.Int32, userId);
            }
            sqlCmd.CreateParameter("@SSOPENED", BuiltInFunction.NullDate());
            sqlCmd.CreateParameter("@SSCLOSED", BuiltInFunction.NullDate());
            sqlCmd.CreateParameter("@SSSTATUS_ID", DbType.Int32, ActivityStatus.Created);
            sqlCmd.CreateParameter("@SSIS_RUNNING_SINCE", DbType.DateTime, DBNull.Value);
            sqlCmd.CreateParameter("@SSIS_RUNNING_AT", DbType.AnsiString, string.Empty);
            if (precedentActivityId == 0) {
                sqlCmd.CreateParameter("@SSPRECEDENT_ACTIVITY_ID", DbType.Int32, DBNull.Value);
            } else {
            ...
            if (dueDate != null) {
                sqlCmd.CreateParameter("@SSDUE_DATE", dueDate.Value);
            }
            if (inboxDetail != null) {
        ...
        public void UpdateActivity(Transaction tran, int? tenantId, int activityId, int? activityDefId, int? processId, string name, int? userId,
            ...
            if (setOpenedNow.HasValue && !setOpenedNow.Value) {
                sqlCmd.CreateParameter("@SSNULLDATETIME", Constants.DefaultDateTime);
            }
            if (status.HasValue) {
            ...
            if (setNextRunNow.HasValue && !setNextRunNow.Value) {
                ...
                if (explicitNextRun.HasValue && explicitNextRun.Value != BuiltInFunction.NullDate()) {
                    sqlCmd.CreateParameter("@SSNEXT_RUN", explicitNextRun.Value);
                } else {
                    sqlCmd.CreateParameter("@SSNEXT_RUN", DbType.DateTime, DBNull.Value);
                }
            }
        ...
        [ExecuteInDatabase(Database = DbKind.Platform, Description = "Requires an activity to exist.")]
        public int CreateProcess(Transaction tran, int tenantId, int createdBy, int processDefId, int parentProcessId, int parentActivityId, int topProcessId, ProcessStatus status) {
            ...
            sqlCmd.CreateParameter("@TENANT_ID", DbType.Int32, tenantId);
            if (createdBy == 0) {
                sqlCmd.CreateParameter("@SSCREATED_BY", DbType.Int32, DBNull.Value);
                sqlCmd.CreateParameter("@SSLAST_MODIFIED_BY", DbType.Int32, DBNull.Value);
            } else {
            ...
            if (parentProcessId == 0) {
                sqlCmd.CreateParameter("@SSPARENT_PROCESS_ID", DbType.Int32, DBNull.Value);
            } else {
                sqlCmd.CreateParameter("@SSPARENT_PROCESS_ID", DbType.Int32, parentProcessId);
            }
            if (parentActivityId == 0) {
                sqlCmd.CreateParameter("@SSPARENT_ACTIVITY_ID", DbType.Int32, DBNull.Value);
            } else {
                sqlCmd.CreateParameter("@SSPARENT_ACTIVITY_ID", DbType.Int32, parentActivityId);
            }
            if (topProcessId == 0) {
                sqlCmd.CreateParameter("@SSTOP_PROCESS_ID", DbType.Int32, DBNull.Value);
            } else {
            ...
            sqlCmd.CreateParameter("@SSLAST_MODIFIED", BuiltInFunction.CurrDateTime());
        ...
        [ExecuteInDatabase(Database = DbKind.Platform, Description = "Requires an activity to exist in REM")]
        ...
        public bool HasActiveActivityInProcess(Transaction tran, int tenantId, ObjectKey activitySSKey, int processId) {
            ...
            return sqlCmd.ExecuteScalar<int>() > 0;
        }
        ...
        private void RequeueEvent(Transaction trans, long id, bool hasError, string tableName) {
            OSTrace.Debug($"[DBRuntimePlatform.Processes > RequeueEvent] id = {id}; hasError = {hasError}; tableName = {tableName}");
            ...
            cmd.CreateParameter("@ID", DbType.Int64, id);
            if (hasError) {
                cmd.CreateParameter("@BACKOFF", DbType.Decimal, RuntimePlatformSettings.BPT.ProcessBackoffConstant.GetValue());
        ...
        public IDataReader DequeueNormalEventForFrontend(Transaction trans, int batch, int activityId, string frontendName) {
            OSTrace.Debug($"[DBRuntimePlatform.Processes > DequeueNormalEventForFrontend] batch = {batch}; activityId = {activityId}; frontendName = {frontendName}");

            Command cmd = trans.CreateCommand(GetDequeueEventSQL());

            cmd.CreateParameter("@BATCHSIZE", DbType.Int32, batch);
            cmd.CreateParameter("@NULLDATE", BuiltInFunction.NullDate());
            cmd.CreateParameter("@ACTIVITYID", DbType.Int32, activityId);
        ...
        public IDataReader DequeueNormalEventForApplication(Transaction trans, int batch, int activityId, ObjectKey applicationKey) {
            OSTrace.Debug($"[DBRuntimePlatform.Processes > DequeueNormalEventForApplication] batch = {batch}; activityId = {activityId}; applicationKey = {applicationKey}");
        ...
        public IDataReader DequeueLightEventForFrontend(Transaction trans, int batch, int activityId, string frontendName) {
            OSTrace.Debug($"[DBRuntimePlatform.Processes > DequeueLightEventForFrontend] batch = {batch}; activityId = {activityId}; frontendName = {frontendName}");
        ...
        public IDataReader DequeueLightEventForApplication(Transaction trans, int batch, int activityId, ObjectKey applicationKey) {
            OSTrace.Debug($"[DBRuntimePlatform.Processes > DequeueLightEventForApplication] batch = {batch}; activityId = {activityId}; applicationKey = {applicationKey}");
