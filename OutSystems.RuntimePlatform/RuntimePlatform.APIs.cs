...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public partial class RuntimePlatformUtils {
...
#if !NETSTANDARD // Asp.Net Core apps don't run scripts in their screens, so no point doing this if the current app is not Traditional
            AppInfo info = AppInfo.GetAppInfo();
            if (info != null) {
                info.CheckPlatformExtensibilityAPIs();
        ...
        private static void InvalidateInjectionCache(AppInfo info) {
            info.OsContext.QueueEspaceToInvalidate(info.ServiceCenterId, 0);
        }
