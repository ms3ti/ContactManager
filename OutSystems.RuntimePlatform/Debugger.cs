...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public class Debugger {
        ...
        private static readonly bool DEBUG_STACK_METHODS = false;
        ...
        public static bool IsRunning {
            get {
                return RunningInfo.DebugMode && !debugSessions.IsEmpty;
            }
        }
        ...
        public static bool DebuggingCurrentThread {
            get {
                ...
                return debugSessions.HasBreakpoints() || (ThreadDataExists && ThreadData.WasSuspended);
            }
        }
        ...
        internal static ThreadData ThreadData {
            get {
        ...
        static readonly Hashtable timers = new Hashtable();
        ...
        public static string GetModuleUrl(string host, string eSpaceName, string /*tenantName*/ _, string personalAreaName) {
...
#region WebMethods
        ...
        public bool HasPermissions(string eSpaceUID, out string errorMessage) {
            errorMessage = "";
#if !INTEGRATION_STUDIO
            var httpContext = httpContextAccessor.Context;
            var authHeader = httpContext.Request.Headers["Authorization"];

            if (!SecurityTokenManager.IsAuthorizationHeaderValid(authHeader, out string token)) {
                throw new Exception("Cannot start debugging because you are using an older version of Service Studio. Update to the latest version of Service Studio and try again.");
            }

            var key = RuntimeSettingsProvider.Instance.Get(RuntimePlatformSettings.Authentication.IdentityServerSignatureKey);

            if (key == null) {
                throw new Exception("Token validation failed");
            }

            // Why are we doing this specifically and not sharing with what is being done for service actions?
            // The current existing code is too specific for ServiceActions use case, to the point where it ignores
            // the Issuer/Audience and validates it manually.
            // Also the SigningKey, it is assuming it is a SymmetricKey (which is not the case here) and it's assuming
            // the key used
            RsaSecurityKey rsa = null;
            using (var keyProvider = new RSACryptoServiceProvider()) {
                keyProvider.FromXmlString(key);
                rsa = new RsaSecurityKey(keyProvider.ExportParameters(false));
            }

            TokenValidationParameters validationParameters =
                new TokenValidationParameters {
                    ValidIssuer = "/server.identity",
                    ValidAudiences = new[] { "runtime.operations.debugger" },
            ...
            try {
                claims = handler.ValidateToken(token, validationParameters, out SecurityToken validatedToken);
            } catch {
                throw new Exception("Token validation failed");
            }

            var allowDebugClaims = claims.Claims.Where(c => c.Type == "allow_debug");

            if (!(allowDebugClaims.Any(c => ObjectKey.Parse(c.Value)?.Equals(ObjectKey.Parse(eSpaceUID)) ?? false))) {
                errorMessage = "ERROR: Debugger_ProducerNoDebugPermission";
                DebuggerTrace.Log("StartDebugSession error :" + errorMessage);
                return false;
            }

            if (ObjectKey.Parse(eSpaceUID) != ObjectKey.Parse(RunningInfo.EspaceUID)) {
                if (!(allowDebugClaims.Any(c => ObjectKey.Parse(c.Value)?.Equals(ObjectKey.Parse(RunningInfo.EspaceUID)) ?? false))) {
                    errorMessage = "ERROR: Debugger_ConsumerNoDebugPermission";
                    DebuggerTrace.Log("StartDebugSession error :" + errorMessage);
                    return false;
                }
            }
        ...
        public string StartDebugSession(string eSpaceUID, string userName, string password, bool breakOnAllErrors, string breakpointsIds) {
            try {
                lock (typeof(Debugger)) {
                    ...
                    if (!RuntimeSettingsProvider.Instance.Get(RuntimePlatformSettings.Misc.EnablePTAVersioning)) {
                    ...
                    //Checks first if it has Authorization header and if not, fallsback to the old mechanism
                    //This is how it will have to be until we figure out how to remove the old validations
                    if (!HasPermissions(eSpaceUID, out string error)) {
                        return error;
                    }
                    ...
                    if (!(breakpointsIds.IsEmpty())) {
                        foreach (string breakpointIdStr in breakpointsIds.Split(',')) {
                            if (BreakpointId.TryParse(breakpointIdStr, out BreakpointId breakpointId)) {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Info Code Smell", "S1133:Deprecated code should be removed", Justification = "Keeping code for backward compatibility and tracing")]
        public void OnBeforeDeploy(string debugSessionToken) {
            OSTrace.Debug($"Obsolete method OnBeforeDeploy called by {RuntimePlatformUtils.GetRequestRealIP(httpContextAccessor.Context.Request)}.");
        }

        internal static void SaveDebugSessionsData() {
            if (!RunningInfo.DebugMode || RunningInfo.ESpaceId == 0) {
                return;
            }
        ...
        public string RemoveBreakpoint(string debugSessionToken, string breakpointId) {
            try {
                ...
                if (BreakpointId.TryParse(breakpointId, out BreakpointId breakpointIdObj)) {
        ...
        public static void Stop(string debugSessionToken) {
            try {
                DebuggerTrace.Log("Stop debugSessionToken=" + (debugSessionToken ?? debugSessions.ActiveDebugSessionTokens.StrCat(", ")));
        ...
        private CallerType GetCallerType(string debugSessionToken) {
            if (IsValidDebugSessionToken(debugSessionToken)) {
        ...
        internal static ThreadData GetThreadData(string threadId) {
            lock (threads) {
                threads.TryGetValue(threadId, out ThreadData result);
                return result;
            }
        }
        ...
        public static BreakpointId Parse(string breakpointIdStr) {
            ...
            BreakpointId.TryParse(breakpointIdStr, out BreakpointId result);
            return result;
        }
        ...
        private static bool HasBreakpointAux(BreakpointId breakpointId, bool isExpressionlessWidget, BreakpointType breakpointType, string extraInfo) {
            try {
                ...
                DebuggerTrace.LogHasBreakpoint(ThreadData, breakpointId, debugSessions, hasBreakpoint);
                return hasBreakpoint;
            } catch (Exception e) {
                DebuggerTrace.Log(e);
                return false;
        ...
        public static void HandleException(Exception e, bool isUnhandled) {
            try {
                if (!IsRunning || !ThreadDataExists || e.GetType() == typeof(DebuggerException) || IsThreadAbortException(e)) {
                    return;
                }
                ...
                IEnumerable<DebugSession> sessions = debugSessions.GetSessionsForCurrentRequest(ThreadData)
        ...
        public static void HandleUnhandledException() {
            try {
                if (ThreadDataExists) {
                    ...
                    IEnumerable<DebugSession> sessions = debugSessions.GetSessionsForCurrentRequest(ThreadData).Where(s => s.ExceptionMode == HandleExceptionMode.BreakOnAll);
                    ThreadData.SendBreakpointEventAndWaitForCommand(
                        sessions,
                        new ExceptionThrownEvent(lastException), null, null);
        ...
        public static void Pop(LocalState stateToPop, bool keepThread) {
            try {
                if (!RunningInfo.DebugMode || !ThreadDataExists) {
                    return;
                }
...
#if !NETSTANDARD
    public class DebuggerSoapEndpoint : WebService {
        private static Debugger DebuggerImpl = new Debugger();
...
#region Component Designer generated code
        ...
        [WebMethod]
        public string StartDebugSession(string eSpaceUID, string userName, string password, bool breakOnAllErrors, string breakpointsIds) {
            return DebuggerImpl.StartDebugSession(eSpaceUID, userName, password, breakOnAllErrors, breakpointsIds);
        }

        [WebMethod]
        public void KillDebugSession(string debugSessionToken) {
            DebuggerImpl.KillDebugSession(debugSessionToken);
        }

        [WebMethod]
        public void StopDebugSession(string debugSessionToken, bool willReconnect) {
            DebuggerImpl.StopDebugSession(debugSessionToken, willReconnect);
        }

        [WebMethod]
        [Obsolete("Endpoint not used anymore on PS 11.27.0+")]
        ...
        public void OnBeforeDeploy(string debugSessionToken) {
            DebuggerImpl.OnBeforeDeploy(debugSessionToken);
        }

        [WebMethod]
        public string SetHandleExceptionMode(string debugSessionToken, bool breakOnAllErrors) {
            return DebuggerImpl.SetHandleExceptionMode(debugSessionToken, breakOnAllErrors);
        }

        [WebMethod]
        public string AddBreakpoints(string debugSessionToken, string breakpointsIds) {
            return DebuggerImpl.AddBreakpoints(debugSessionToken, breakpointsIds);
        }

        [WebMethod]
        public string RemoveBreakpoint(string debugSessionToken, string breakpointId) {
            return DebuggerImpl.RemoveBreakpoint(debugSessionToken, breakpointId);
        }


        [WebMethod]
        public string ClearBreakpoints(string debugSessionToken) {
            return DebuggerImpl.ClearBreakpoints(debugSessionToken);
        }

        [WebMethod]
        public string SuspendAllThreads(string debugSessionToken) {
            return DebuggerImpl.SuspendAllThreads(debugSessionToken);
        }

        [WebMethod]
        public string ContinueThread(string debugSessionToken, string threadId) {
            return DebuggerImpl.ContinueThread(debugSessionToken, threadId);
        }

        [WebMethod]
        public string Stop(string debugSessionToken, string threadId) {
            return DebuggerImpl.Stop(debugSessionToken, threadId);
        }

        [WebMethod]
        public string RunTo(string debugSessionToken, string threadId, string breakpointId) {
            return DebuggerImpl.RunTo(debugSessionToken, threadId, breakpointId);
        }

        [WebMethod]
        public string StepInto(string debugSessionToken, string threadId) {
            return DebuggerImpl.StepInto(debugSessionToken, threadId);
        }

        [WebMethod]
        public string StepOver(string debugSessionToken, string threadId) {
            return DebuggerImpl.StepOver(debugSessionToken, threadId);
        }

        [WebMethod]
        public string StepOut(string debugSessionToken, string threadId) {
            return DebuggerImpl.StepOut(debugSessionToken, threadId);
        }

        [WebMethod]
        public string Evaluate(string debugSessionToken, string threadId, string vars) {
            return DebuggerImpl.Evaluate(debugSessionToken, threadId, vars);
        }
    }
    ...
    class EventQueue {
        ...
        private void RemoveOldEvents() {
            ThreadUtils.SetupCurrentThreadCulture();
            while (true) {
...
#region Debug events
    ...
    public class ThreadEndEvent : DebuggerEvent {
        ...
        public ThreadEndEvent(string threadId, DebugSession affinedSession)
            ...
            this.userName = affinedSession?.UserName;
        }
    }
    ...
    public class ApplicationIsBeingRedeployedEvent : DebuggerEvent {
        readonly bool hasSuspendedThreads;
        readonly bool fromRedeployEvent;
        ...
        class LongLivedRequestData {
            ...
            private static LongLivedRequestData InnerGet(ThreadData threadData) {
                ...
                lock (persistedData) {
                    ...
                    if (persistedData.TryGetValue(Debugger.ProcessInfo.TopProcessId, out Dictionary<int, LongLivedRequestData> dataForProcess)) {
                ...
                if (isLongLived) {
                    ...
                    if (ThreadWasSuspended) {
                        lock (persistedData) {
                            ...
                            if (!persistedData.TryGetValue(processInfo.TopProcessId, out Dictionary<int, LongLivedRequestData> dataForProcess)) {
            ...
            private LongLivedRequestData Duplicate(string newRequestId) {
                var result = new LongLivedRequestData(isLongLived, newRequestId) {
                    // copy data
                    GenerateContinueEventOnNextBreakpoint = this.GenerateContinueEventOnNextBreakpoint
                };
                ...
                foreach (XmlElement process in root.SelectNodes("Process")) {
                    ...
                    foreach (XmlElement activity in process.SelectNodes("Activity")) {
                        ...
                        if (!runToBreakpoint.IsEmpty()) {
                            if (BreakpointId.TryParse(runToBreakpoint, out BreakpointId breakpointId)) {
    ...
    public class StackManager : IDebuggerSerializable {
        ...
        public StackManager CloneStack() {
            return new StackManager {
                callStack = new List<LocalState>(callStack),
        ...
        private int GetESpaceIdByName(string eSpaceName) {
            var appInfo = AppInfo.GetAppInfo();
            var currenteSpaceId = appInfo.eSpaceId;
            var currentESpaceName = appInfo.eSpaceName;

            if (currentESpaceName.EqualsIgnoreCase(eSpaceName)) {
                return currenteSpaceId;
            }

            using var trans = DatabaseAccess.ForRuntimeDatabase.GetReadOnlyTransaction();
            return DBRuntimePlatform.Instance.GetEspaceId(trans, eSpaceName);
        }
            ...
            switch (varType) {
                ...
                case "site":
                    if (varName.IndexOf(".") >= 0) {
                        // Site properties are accessed with "ESPACENAME.SITEPROPNAME".
                        var parts = varName.Split('.');
                        string ownerEspaceName = parts[0];
                        string sitePropertyName = parts[1];
                        var ownerESpaceId = GetESpaceIdByName(ownerEspaceName);
                        varValue.Value = AppInfo.GetAppInfo().Espace(ownerESpaceId).EspaceSiteProperties.GetSafeByNameForDebugger(sitePropertyName);
                    }
                    break;
                ...
                case "local":
                    ...
                    if (page != null) {
                        ...
                        if (controlId != null) {
                            if (!varValue.Found) {
                                varValue.Value = new Web.UnknownWidgetRtWidget(controlId);
                            }
                            if (varValue.Value is Web.IRtWidget widget) {
                                widget.Id = controlId;
...
#if !NETSTANDARD
            Web.IScreen page = (Web.IScreen)page_;
    ...
    public class VarValue {
        ...
        private readonly StackManager stackMngr;
        private readonly int stackLevel;

        static readonly string toXmlMethod;
        static readonly string evaluateFieldsMethod;
        ...
        public void EvaluateFields(object parent, string baseName, string fields) {
            try {
                ...
                if (varValue is CaseInsensitiveHashtable ht) {
        ...
        private void Store(XmlElement variable, object val, int detailLevel) {
            ...
            // try calling the ToXml method. If that fails, return the variable valueas a string
                ...
                variable.InnerText = ValToString(detailLevel, optimized, val, out bool wasTrimmed);
                if (wasTrimmed) {
        ...
        public static string ValToString(int detailLevel, bool optimized, object val) {
            return ValToString(detailLevel, optimized, val, TypeKind.None, out _);
        }
        ...
        public static string ValToString(int detailLevel, bool optimized, object val, TypeKind kind) {
            return ValToString(detailLevel, optimized, val, kind, out _);
        }
        ...
        public static string ValToString(int detailLevel, bool optimized, object val, TypeKind kind, out bool wasTrimmed) {
            ...
            if (val is DateTime time) {
                if (kind == TypeKind.Time) {
                    return "#" + BuiltInFunction.TimeToText(time) + "#";
                }
                if (kind == TypeKind.Date) {
                    return "#" + BuiltInFunction.DateToText(time) + "#";
                }
                return "#" + BuiltInFunction.DateTimeToText(time) + "#";
            }
            ...
            if (IsByteArray(val)) {
                ...
                if (data.Length == 1) {
                    ...
                    return "BinaryData (" + data.Length.ToString() + " bytes)";
                }
            }
...
#region Xml construction functions
        ...
        public static XmlAttribute AppendOptimizedAttribute(XmlElement root, string name) {
            ...
            attr.Value = ValToString(-1, true, null, out bool wasTrimmed);
            return attr;
        }
        ...
        public static XmlAttribute AppendAttribute(XmlElement root, string name, object val, int detailLevel, TypeKind kind) {
            ...
            if (name.StartsWith("debug.")) {
                ...
                attr.Value = ValToString(detailLevel, false, val, kind, out bool wasTrimmed);
                if (wasTrimmed) {
        ...
        public static XmlElement AppendAttributeAsChild(XmlElement root, string name, object val, int detailLevel) {
            ...
            var valueStr = VarValue.ValToString(detailLevel, false, val, out bool wasTrimmed);
        ...
        public static bool FieldIsOptimized(object parent, string fieldInLowerCase) {
            if (parent is RecordList parentRecList && parentRecList.OptimizedFields != null) {
    ...
    public class VarsBag : IVarsBag {
        ...
        private Dictionary<string, int> varNamesDictionary = null;
        ...
        protected void Init(string[] varNames, string[] varRtNames) {
            if (varNames.Length == varRtNames.Length) {
                ...
                origin = this;
                varNamesDictionary = new Dictionary<string, int>(varNames.Length, StringComparer.InvariantCultureIgnoreCase);
                for(int i = 0; i < varNames.Length ; i++) {
                    if (!varNamesDictionary.ContainsKey(varNames[i])) {
                        varNamesDictionary.Add(varNames[i], i);
                    }
                }
            }
        }
        ...
        private int GetVariableIndex(string varName) {
            ...
            if (varNamesDictionary.TryGetValue(varName, out int value)) {
                return value;
            } else {
                //Fallback to the old behavior in a scenario where the variables may not have been correctly initialized
                DebuggerTrace.Log($"Falling back to old GetVariableIndex implementation for variable {varName}.");
                varName = varName.ToLowerInvariant();
                for (int i = 0; i < varNames.Length; i++) {
                    if (varNames[i].ToLowerInvariant() == varName) {
                        return i;
                    }
                }
                return -1;
        ...
        public object GetVariableValue(string varName) {
            ...
            MemberInfo varField = GetVariableMember(GetVariableIndex(varName), out object targetObject);
    ...
    public class LocalState {
        private readonly string eSpaceName;
        private readonly BreakpointId functionId;
        private readonly string functionType;
        ...
        public LocalState(string eSpaceName, string functionIdStr, string functionName, string functionType, object page, params IVarsBag[] list) {
            ...
            if (BreakpointId.TryParse(functionIdStr, out BreakpointId breakpointId)) {
...
#if !NETSTANDARD
            if (obj is Web.IListRecordRtWidget list) {
                if (fieldName.ToLower() == "list") {
                    list.List = (IOSList)fieldValue;
                }
                return true;
            } else if (obj is Web.IRecordRtWidget record) {
                if (fieldName.ToLower() == "record") {
                    record.Record = fieldValue;
                }
                return true;
            } else if (obj is Web.IListBoxRtWidget listBox) {
                if (fieldName.ToLower() == "list") {
                    listBox.List = (IOSList)fieldValue;
                }
                return true;
            } else if (obj is Web.IRtCustomWidget customWidget) {
        ...
        private static BreakpointId GetBreakpointId(XmlElement elem, string attrName) {
            ...
            if (breakpointIdStr.IsEmpty()) {
                ...
                BreakpointId.TryParse(breakpointIdStr, out BreakpointId breakpointId);
                return breakpointId;
            }
        }
    }
    ...
    public class DebuggerEventsData {
        ...
        internal DebuggerEventsData(IHttpContext httpContext, string sessionToken) {
            _debugSessionToken = sessionToken;
            this.httpContext = httpContext;
            ESpaceUID = ObjectKey.Parse(httpContext.Request.Params["eSpaceUID"]);
            RefreshClientIp(httpContext);
        }
        ...
        internal void RefreshClientIp(IHttpContext context) {
            lastConnectedIp = RuntimePlatformUtils.GetRequestRealIP(context.Request);
        }

        internal ObjectKey ESpaceUID { get; }
        ...
        public override bool Equals(object obj) {
            ...
            return ((_debugSessionToken == otherObj._debugSessionToken) && (ESpaceUID == otherObj.ESpaceUID));
        }
    ...
    public class DebuggerEvents : IHttpHandler {
        ...
        private class Clients {
            ...
            public DebuggerEventsData Add(HttpContext context, string sessionToken, bool useShortRequests) {
                return Add(httpContextAccessor.Context, sessionToken, useShortRequests);
            }

            public DebuggerEventsData Add(IHttpContext context, string sessionToken, bool useShortRequests) {
                if (useShortRequests) {
                    lock (this) {
                        DebuggerEventsData result;

                        if (sessionTokenToClientAndExpirationDateNode.TryGetValue(sessionToken, out LinkedListNode<ClientAndExpirationDate> previousClient)) {
        ...
        private bool IsClientConnected(IHttpContext context) {
            return context.Response.IsClientConnected;
        }

        private void SendData(IHttpContext context, string data) {
        ...
        private void SendEvent(IHttpContext context, DebugSession debugSession, string debugSessionToken, DebuggerEvent _event, long eventId) {
            if (_event != null) {
        ...
        private bool GetAndSendEvent(IHttpContext context, DebuggerEventsData eventsData, int getEventDefaultTimeoutMultiplier,
            ...
            DebuggerEvent @event = Debugger.GetDebugEvent(eventsData.DebugSessionToken, lastEventReceived.Id,
                getEventDefaultTimeoutMultiplier * Debugger.GET_DEBUG_EVENT_TIMEOUT_IN_MS, out long eventId);
        ...
        public void ProcessRequest(HttpContext context) {
            ProcessRequest(httpContextAccessor.Context);
        }
        ...
        public void ProcessRequest(IHttpContext context) {
            ...
            bool.TryParse(context.Request.Params["isReconnect"], out bool isReconnect);
    ...
    public class DebuggerTrace {
        ...
        private static readonly bool PrintHasBreakpointInformation = false;
        private static readonly bool PrintHasBreakpointVerboseInformation = false;

        private static string Header {
            get {
                string requestPath = string.Empty;

                if (HttpService.Context?.Request?.Path != null) {
                    requestPath = HttpService.Context.Request.Path;
                }
        ...
        public static void LogHasBreakpoint(ThreadData threadData, BreakpointId breakpoint, DebugSessions sessions, bool hasBreakpoint) {
            ...
            foreach (DebugSession session in sessions.GetSessionsForCurrentRequest(threadData)) {
    ...
    public class DebugSessions {
        ...
        public IEnumerable<DebugSession> GetSessionsForCurrentRequest(ThreadData threadData) {
            lock (this) {
                if (debugSessions.IsEmpty()) {
                    return Enumerable.Empty<DebugSession>();
                } else if (threadData.DebugSessionToken.IsEmpty()) {
                    return debugSessions.Where(s => s.RequestAppliesToSession()).ToArray();
                } else {
                    return debugSessions.Where(s => s.Token == threadData.DebugSessionToken).ToArray();
                }
            }
        }
    ...
    public class DebugSession {
        ...
        private string GetRequestRealIP(IHttpRequest request) {
            return RuntimePlatformUtils.GetRequestRealIP(request);
        }

        private static bool IsWebServiceRequest() {
            return AppInfo.GetAppInfo().OsContext.IsWebServiceRequest;
        }

        private static bool IsServiceAction() {
            return AppInfo.GetAppInfo().OsContext.IsServiceAction;
        }

        private static bool IsNewRuntimeApplication() {
            return AppInfo.GetAppInfo().IsMobileRuntime;
        }

        private static bool AllowDebuggingTraditionalAppsFromAnyIp() {
            return RuntimePlatformSettings.Debug.AllowDebuggingTraditionalAppsFromAnyIp.GetValue();
        }

        public bool RequestAppliesToSession() {
            var request = httpContextAccessor.Context.Request;

            var originalIp = GetRequestRealIP(request);
            var requestIp = request.UserHostAddress;
            var header = request.Headers.GetValueOrDefault(HttpHeaderXForwardedFor);
            string details = $"originalIp: [{originalIp}] requestIp: [{requestIp}] header: [{header}]";

            // requests to web services apply to all sessions
            if (IsWebServiceRequest()) {
                DebuggerTrace.Log($"{details} IsWebServiceRequest");
                return true;
            }

            // requests to service actions apply to all sessions
            if (IsServiceAction()) {
                DebuggerTrace.Log($"{details} IsServiceAction");
                return true;
            }
            ...
            if (IsNewRuntimeApplication()) {
                DebuggerTrace.Log($"{details} IsNewRuntimeApplication");
                return true;
            }

            // local requests apply to all sessions
            if (IsLocalConnection(originalIp)) {
                DebuggerTrace.Log($"{details} IsLocalConnection");
                return true;
            }

            // remote requests apply only to clients with the same ip
            var client = clients.FirstOrDefault(c => c.LastConnectedIp == originalIp);
            if (client != null) {
                DebuggerTrace.Log($"{details} Client exists: {client.LastConnectedIp} {client.DebugSessionToken}");
                return true;
            }

            if (AllowDebuggingTraditionalAppsFromAnyIp()) {
                DebuggerTrace.Log($"{details} AllowDebuggingTraditionalAppsFromAnyIp enabled");
                return true;
            }

            string clientIps = string.Join(Environment.NewLine, clients.Select(c => $"{c.LastConnectedIp} {c.DebugSessionToken}"));
            DebuggerTrace.Log($"{details} Request doesn't apply to any client:{Environment.NewLine} {clientIps}");
            return false;
        }
        ...
        private DebugSession(XmlElement elem) {
            ...
            foreach (XmlElement breakpointElem in elem.SelectSingleNode("Breakpoints").SelectNodes("Breakpoint")) {
                if (BreakpointId.TryParse(breakpointElem.InnerText, out BreakpointId breakpointId)) {
                    breakpoints.Add(breakpointId);
                }
            }
    ...
    public static class DebuggerHelper {
        ...
        public static void SetCurrentCallContextId(string callContextId) {
            if (RunningInfo.DebugMode) {
                if (int.TryParse(callContextId, out int id)) {
        ...
        public static void SetCurrentRunToBreakpoint(string breakpointStr) {
            if (RunningInfo.DebugMode) {
                if (BreakpointId.TryParse(breakpointStr, out BreakpointId parsed)) {
        ...
        public static void SetCurrentStopImmediately(string stopImmediatelyStr) {
            if (RunningInfo.DebugMode) {
                if (bool.TryParse(stopImmediatelyStr, out bool parsed)) {
