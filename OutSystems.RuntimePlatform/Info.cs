...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public class EspaceProperties {
		...
		public bool EnableSchemaCompliance {
            get {
                return RuntimePlatformSettings.Module.EnableSchemaCompliance.GetValue();
            }
        }
    ...
    public class AjaxEventContextInfo {
        ...
        internal static AjaxEventContextInfo GetAjaxEventContextInfo() {
    ...
    /// <summary>
    /// Class that is used to store the information about the application.
    /// THIS CLASS IS USED EXTERNALLY. WE CANNOT BREAK THIS INTERFACE.
    /// </summary>
    // DO NOT REMOVE sealed qualifier
    public sealed class AppInfo : IAppInfo, IDisposable {
        private const string APP_CACHE_FILE_PREFIX = "appCacheFile_";

        private const string HttpItemIsLoadingScreen = "osIsLoadingScreen";
        private const string HttpItemCurrentPTAUserName = "osCurrentPTAUserName";
        ...
        private HeContext heContext = null;
        ...
        private (EspaceProperties properties, ExtensionPropertiesCache extensionProperties) CommonInitProperties() {
            ...
            // Init the properties
            var properties = new EspaceProperties();
            var extensionProperties = new ExtensionPropertiesCache(this);
            LicenseParameters = new LicenseParameters(_eSpaceUID, _eSpaceId, null, false);

            return (properties, extensionProperties);
        }

        public EspaceInfo Espace(int espaceId) {
            if (!_espaces.TryGetValue(espaceId, out EspaceInfo espaceInfo)) {
                lock (_espaces) {
        ...
        public string GetWatermark() {
            if (string.IsNullOrEmpty(LicenseParameters.DevelopmentLimitWatermark)) {
                ...
                var watermark = LicenseParameters.DevelopmentLimitWatermark;
                watermark = watermark.Replace("[SessionId]", ""); // Old code was replacing this token in the watermark test. Just removing it for security reasons.
                return watermark;
            }
        }
        ...
        public string GetCallbackDebugInformation() {
            ...
            ssInformation += Callbacks.CallbackResultStorage.GetPropertyText("eSpace Id", eSpaceId);
            ssInformation += Callbacks.CallbackResultStorage.GetPropertyText("eSpace Name", eSpaceName);
            ssInformation += Callbacks.CallbackResultStorage.GetPropertyText("eSpace Version Id", eSpaceVersionId);
            ssInformation += Callbacks.CallbackResultStorage.GetPropertyText("User Id", OsContext?.Session?.UserId ?? 0);
            ssInformation += Callbacks.CallbackResultStorage.GetPropertyText("User Name", OsContext?.Session?.UserName ?? "");
            ssInformation += Callbacks.CallbackResultStorage.GetPropertyText("OsContext.ResponseDisabledFeedback", OsContext?.ResponseDisabledFeedback ?? false);
            ssInformation += Callbacks.CallbackResultStorage.GetPropertyText("OsContext.ResponseDisabledTaskBox", OsContext?.ResponseDisabledTaskBox ?? false);
        ...
        #endregion Callback debug Information
        ...
        public bool SelectiveLoggingEnabled {
            get {
                return LicenseParameters?.SelectiveLoggingEnabled ?? false;
            }
        }
        ...
        public bool MultilingualEnabled {
            get {
                return LicenseParameters?.MultilingualEnabled ?? false;
            }
        }
        ...
        internal bool PlatformExtensibilityAPIsEnabled {
            get {
                return LicenseParameters?.PlatformExtensibilityAPIsEnabled ?? false;
            }
        }

        internal void CheckPlatformExtensibilityAPIs() {
            if (RuntimePlatformSettings.Application.IsInIsolatedDB.GetValue()) {
                throw new InvalidOperationException("Platform Extensibility APIs are not supported in modules that are not deployed in the System DB.");
            }
        ...
        internal void CheckMultitenancy() {
            if (LicenseParameters != null && !LicenseParameters.MultitenancyAllowed) {
        ...
        private Func<IEnumerable<StaticEntity>> _getStaticEntities;

        public IEnumerable<StaticEntity> StaticEntitiesKeys {
            get {
                return _getStaticEntities != null ? _getStaticEntities() : Enumerable.Empty<StaticEntity>();
            }
        }
        ...
        public bool IsLoadingScreen {
            get => Convert.ToBoolean(HttpService.Context.Items["osIsLoadingScreen"]);
            set => HttpService.Context.Items[HttpItemIsLoadingScreen] = value;
        }
        ...
        public string PtaUserName => (string)HttpService.Context.Items[HttpItemCurrentPTAUserName];
        ...
        public HeContext OsContext {
            get {
                ...
                return ServiceLocator.GetService<HeContext>();
            }
            set {
                heContext = value;
            }
            ...
            get {
                ...
                if (!isApplicationEnabled || !isTenantActive) {
                    if (!isTenantActive) {
                        ...
                        OsContext.Session.InternalSession.Clear();
        ...
        public AppInfo(ESpaceInfoForAppStart eSpaceInfo) {
            ...
            _defaultTenantId = eSpaceInfo.DefaultTenantId;
            _eSpaceVersionId = eSpaceInfo.ESpaceVersionId;
            ...
            if (eSpaceInfo.ESpaceId <= 0 || eSpaceInfo.ESpaceName.IsEmpty()) {
                throw new InvalidOperationException($"Invalid eSpace info provided to AppInfo : eSpace Id: {eSpaceInfo.ESpaceId}, eSpace Name: '{eSpaceInfo.ESpaceName}'");
            }

            (Properties, ExtensionProperties) = CommonInitProperties();
        }

        public AppInfo(ISettingsProvider settingsProvider, IApplicationStaticProperties applicationProperties) {
            // TODO: For now always assume the first for application wide parameters...
            IModuleStaticProperties module = applicationProperties.Modules.Values.First();
            _eSpaceId = module.ESpaceId;
            _eSpaceUID = module.ESpaceKey;
            _eSpaceUIDAsString = ObjectKeyUtils.DatabaseValue(_eSpaceUID);
            _eSpaceName = module.ESpaceName;
            _isMultitenant = applicationProperties.IsMultiTenant;
            _isMobileRuntime = true;

            _getStaticEntities = () => {
                var staticEntities = new HashSet<StaticEntity>();
                var producersKeys = new HashSet<ObjectKey>();
                return applicationProperties.Modules.Values.SelectMany(m => m.GetStaticEntities(staticEntities, producersKeys));
            };

            _applicationUIDAsKey = ObjectKey.Parse(settingsProvider.Get(RuntimePlatformSettings.Application.Key));
            _applicationUID = ObjectKeyUtils.DatabaseValue(_applicationUIDAsKey);

            // Set user provider eSpace information
            if (applicationProperties.UserProviderKey != null) {
                _userProviderEspaceKey = applicationProperties.UserProviderKey;
                _userProviderName = applicationProperties.UserProviderName;
            } else {
                // The current tenant is its own user provider
                _userProviderEspaceKey = _eSpaceUID;
                _userProviderName = _eSpaceName;
            ...
            _defaultTenantId = applicationProperties.DefaultTenantId;
            _eSpaceVersionId = RuntimePlatformSettings.Module.ForKey(_eSpaceUID).VersionId.GetValue();

            _dataBaseConfigurationDetails = applicationProperties.DataBaseConfigurationDetails;
            _catalogName = applicationProperties.CatalogName;
            _cacheInvalidationURLSuffix = module.CacheInvalidationSuffix;
            (Properties, ExtensionProperties) = CommonInitProperties();
        }
        ...
        private InjectionCache BuildInjectionCache() {
            if (RuntimePlatformSettings.Application.IsInIsolatedDB.GetValue()) {
                // Callbacks are not supported in isolated DBs.
                // Since the callback entities are not created in isolated DBs, we need to prevent
                // the application runtime from executing queries that use those Db tables.
        ...
        internal static void GetAppInfoInformation(out int espaceid, out int tenantid, out string sessionid, out string sessionGuid, out int userid) {
            espaceid = 0;
            tenantid = 0;
            userid = 0;
            sessionid = "";
            ...
            var info = singleton ?? ServiceLocator.GetService<AppInfo>();

            if (info != null) {
                if (info != null) {
                    var context = info.OsContext;
                    ...
                    try { userid = context.Session.UserId; } catch { }
                }
            }
        }

        private static volatile AppInfo singleton = null;
...
#if !NETSTANDARD
        [Obsolete("Use the SetAppInfo(AppInfo) method.")]
        ...
        public static void SetAppInfo(AppInfo appInfo) {
            singleton = appInfo; // TODO move this our of the singleton pattern when we have dependency injection ServiceLocators
        }
        ...
        public string FeatureTogglesStateDuringCompilation { get; }
    }
    ...
    public class WSEnhancementsElementWithSoapHeaders : WSEnhancementsElement {
        ...
        private Func<System.Web.Services.Protocols.SoapUnknownHeader[]> _getSoapHeaders;

        public System.Web.Services.Protocols.SoapUnknownHeader[] SOAPHeaders {
            get {
        ...
        public static System.Web.Services.Protocols.SoapUnknownHeader[] GetHeaders(HeContext context, string name, bool isWebService, bool forExtension) {
        ...
        public static void SetHeaders(HeContext context, string name, bool isWebService, bool forExtension, System.Web.Services.Protocols.SoapUnknownHeader[] headers) {
            SetHeadersDataSource(context, name, isWebService, forExtension, () => headers);
        }

        public static void SetHeadersDataSource(HeContext context, string name, bool isWebService, bool forExtension, Func<System.Web.Services.Protocols.SoapUnknownHeader[]> headersDataSource) {
            string key = GetHeadersKey(isWebService, forExtension, name);
    ...
    public class WSEnhancementsElement {
        ...
        public static void setWSCredentials(HeContext context, string ssWebServiceName, System.Web.Services.Protocols.SoapHttpClientProtocol ws) {
        ...
        public static void setWSProxy(HeContext context, string ssWebServiceName, System.Web.Services.Protocols.SoapHttpClientProtocol ws) {
