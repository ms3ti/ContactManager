...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    internal class DBRuntimePlatformSqlServer : DBRuntimePlatform {
        ...
        public override string GetSelectForValues(IEnumerable<int> ids, string idColumnName) {
            var recordIdValues = string.Join(",", ids.Select(value => $"({value})"));
            return $@"SELECT {idColumnName}
                        FROM
                        (
                            VALUES {recordIdValues}
                        ) SOME_NAME({idColumnName})";
        }
        #endregion
        ...
        protected override string GetTaskboxActivityCountSelectClauseSql(string lastSeenActivityQuery) {
            return $@"
                    DECLARE @LASTSEENACTIVITYID INT;
                    SELECT @LASTSEENACTIVITYID = {lastSeenActivityQuery}
