...
namespace OutSystems.RuntimePublic {
    ...
    public static class Session {
...
#if !NETSTANDARD // In .Net Core apps never try to access the session database
            string sessionId =GetCurrentSession();
            if (!sessionId.IsNullOrEmpty()) {
                return DatabaseAccess.ForSession.DatabaseServices.SessionService.GetUserBySessionId(Application.Session.Persistent.Data.SessionIdParser.Parse(sessionId, RuntimeSettingsProvider.Instance));
            }
#endif
            return 0;
        }
        ...
        private static string GetCurrentSession() {
            HttpService.Context.Request.Cookies.TryGetValue(Application.Session.Persistent.Cookies.CookieActions.GetSessionCookieName(), out var cookie);
        ...
        public static void GetMobileAppLoginId(out string loginId) {
            var loginInfo = (AppInfo.GetAppInfo().OsContext.Session.InternalSession as MobileSessionInfo)?.LoginInfo;
            loginId = loginInfo != null && loginInfo.UserId != 0 ? loginInfo.LoginId : null;
        }
    }
