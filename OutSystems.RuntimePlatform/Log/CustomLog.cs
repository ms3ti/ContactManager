...
namespace OutSystems.HubEdition.RuntimePlatform.Log {
    ...
    public class CustomLog {
        ...
        public static void SynchronousWrite(List<CustomLog> logList) {
            // these should be provided as dependency injection instead, but not breaking the public API for now
            var databaseAccessService = new RuntimePlatformServices.DefaultDatabaseAccessService();
            var logDatabaseService = new RuntimePlatformServices.RuntimeLoggingDatabaseService(databaseAccessService);

            IEnumerable<DataTable> tables = ConvertToDataTable(logList);
            foreach (CustomLogDataTable tab in tables) {
                var databaseAccessProvider = logDatabaseService.GetProviderFor(tab);
        ...
        private static IEnumerable<DataTable> ConvertToDataTable(IEnumerable<CustomLog> logList) {
            ...
            foreach (CustomLog log in logList) {
                ...
                if (!tables.TryGetValue(key, out CustomLogDataTable table)) {
