...
namespace OutSystems.HubEdition.RuntimePlatform.Web {
    public class Dummy {
        public void AlsoDummy() { }
    }
}

namespace OutSystems.HubEdition.RuntimePlatform {

    public abstract class BaseAppUtils : IAppUtils {

        protected BaseAppUtils() {
        }

        protected IHttpContext GetHttpContext(HeContext heContext) {
            return heContext.CurrentHttpContext;
        }
...
#if !NETSTANDARD // Only relevant in Traditional Web apps
        /// <summary>
        ...
        protected static Regex NotAllowedTableNamesRegex { get { return _notAllowedTableNamesRegex ??= new Regex("(^|\\s|\\.)(\\[|\"|)(ossys_|osltm_|oslog_|osevt_|osusr_|osext_)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled); } }

        private static Regex _entitiesInOrderByRegex;
        protected static Regex EntitiesInOrderByRegex { get { return _entitiesInOrderByRegex ??= new Regex(@"(\{\w+\}|\w+\.)", RegexOptions.CultureInvariant | RegexOptions.Compiled); } }

        private static Regex _dynOrderByValidateRegex;
        protected static Regex DynOrderByValidateRegex { get { return _dynOrderByValidateRegex ??= new Regex(@"((((\{\w+\})|(\w+)).)?((\[\w+\])|(\w+))(( )+(desc|asc))?)(( )*,( )*(((\{\w+\})|(\w+)).)?((\[\w+\])|(\w+))(( )+(desc|asc))?)*", RegexOptions.IgnoreCase); } }

        private static Regex _entityAttributesRegex;
        protected static Regex EntityAttributesRegex { get { return _entityAttributesRegex ??= new Regex(@"((\{\w+\}).(\[\w+\]))*", RegexOptions.IgnoreCase); } }

        private static Regex _aliasAttributesRegex;
        protected static Regex AliasAttributesRegex { get { return _aliasAttributesRegex ??= new Regex(@"(\w+)(\s*\.\s*((\[\w+\])|(\w+)))", RegexOptions.CultureInvariant | RegexOptions.Compiled); } }

        private static Regex _attributesRegex;
        protected static Regex AttributesRegex { get { return _attributesRegex ??= new Regex(@"(\[\w+\])", RegexOptions.CultureInvariant | RegexOptions.Compiled); } }

        private static Regex _orderByDirectionRegex;
        protected static Regex OrderByDirectionRegex { get { return _orderByDirectionRegex ??= new Regex(@"\b(ASC|DESC)$", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.ExplicitCapture); } }

        private static Regex _controlCharsRegex;
        protected static Regex ControlCharsRegex { get { return _controlCharsRegex ??= new Regex(@"[^\P{Cc}\s]", RegexOptions.Compiled); } }
        ...
        public bool CheckForSelectStatement(string sql) {
            if (!StringUtils.IsSelectStatement(sql)) {
                throw new DataBaseException("Could not calculate the query count.Actual SQL used was: " + sql);
            }
            return true;
        }
        ...
        public static string ReplaceEntityReferencesAndAttributesInOrderBy(IDMLIdentifiers dmlIdentifiers, string paramValue, string[] entityNames, string[] entityAliases, IDictionary<string, string>[] entityNamesToAttributes) {
            ...
            if (paramValue != "") {
                ...
                if (mc.Count != 1 || mc[0].Value != paramValue) {
                    throw new DataBaseException($"Invalid syntax in 'Dynamic Order By' parameter: '{paramValue}'."
                        + " To reference entities write it between curly brackets and to reference an entity attribute write it between square brackets"
                        + " (e.g. {User}.[PhoneNumber]).");
                }
            }

            MatchEvaluator meReplaceEntityName = match => ReplaceEntityNameInOrderBy(match, entityNames, entityAliases);
            MatchEvaluator meReplaceEntityAttributeName = match => ReplaceEntityAttributesName(dmlIdentifiers, match, entityAliases, entityNamesToAttributes);
            MatchEvaluator meReplaceAttributeName = match => ReplaceAttributeName(dmlIdentifiers, match);
        ...
        private static string ReplaceEntityAttributesName(IDMLIdentifiers dmlIdentifiers, Match m, string[] entityAliases, IDictionary<string, string>[] entityAttributesNamesToDatabaseNames) {
            ...
            // Escape the attribute name
            return dmlIdentifiers.EscapeIdentifier(entityAlias) + "." + dmlIdentifiers.EscapeIdentifier(entityPhysicalAttributeName);
        }
        ...
        protected static string EntityToTablename(string entity, Dictionary<string, string> entitiesTablenames) {
            entitiesTablenames.TryGetValue(entity, out string tablename);
            return tablename;
        }
        ...
        protected static string EntityAttributeToColumnName(Pair<string, string> entityAttribute, Dictionary<Pair<string, string>, string> entityAttributenames) {
            entityAttributenames.TryGetValue(entityAttribute, out string columnName);
            return columnName;
        }
