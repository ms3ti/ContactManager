...
namespace OutSystems.HubEdition.RuntimePlatform {
    ...
    public static class BuiltInFunction {
        ...
        private const string HttpItemCurrentPTAName = "osCurrentPTAName";
        ...
        public static string EncodeSql(string s) {
            return DatabaseAccess.ForRuntimeDatabase.DatabaseServices.DMLService.EscapeTextValue(s);
        }
        ...
        public static string GetDatabaseProvider() {
            try {
                return DatabaseAccess.ForRuntimeDatabase.DatabaseServices.DatabaseConfiguration.DatabaseProvider.Key.Serialize();
            } catch {
                return string.Empty;
            }
        }
        ...
        public static string GetUserAgent() {
            try {
                return HttpService.Context.Request.UserAgent ?? string.Empty;
            } catch {
                return string.Empty;
            }
        }
...
#if !NETSTANDARD // when used in server side on Mobile / Reactive applications we never have a Screen associated
            try {
                var page = AppInfo.GetAppInfo().OsContext.CurrentScreen as IBookmarkableURL;
                if (page != null) {
                    var url = page.GetBookmarkableURL();
                    if (url != null) {
                        return RuntimePlatformUtils.CheckUriSecurity(HttpService.Context.Request, url);
                    }
                }
            } catch { }
#endif
        ...
        public static string AddPersonalAreaToURLPath(string url) {
            try {
                ...
                string ptaname = "/" + HttpService.Context.Items.GetValueOrDefault(HttpItemCurrentPTAName, "");
...
#if !NETSTANDARD
                //Used only on IIS, for .net core, OsISAPIFilter isn't used, so it will always be disabled
...
#if !NETSTANDARD // No Ajax in Asp.Net Core apps
        ...
        public static bool CheckRole(int roleId, int userId) {
            ...
            using var tran = RuntimePlatformSettings.Misc.ReadUncommittedOrReadCommittedWithMARS.GetValue() ? DatabaseAccess.ForSystemDatabase.GetRequestTransaction() : DatabaseAccess.ForSystemDatabase.GetReadOnlyTransaction();
