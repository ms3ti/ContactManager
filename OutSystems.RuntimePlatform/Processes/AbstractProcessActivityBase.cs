    ...
    public abstract partial class ProcessBase {
        public abstract class AbstractProcessActivityBase : BPMRuntime, IProcessActivity {
            ...
            public virtual void PushDebuggerStack() {
                ...
                int parentActivityId, parentProcessId;
                string key, processDetail;
                bool hasParent;
                ...
                using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                    ...
                    hasParent = DBRuntimePlatform.Instance.HasParentProcess(trans, CurrentProcessInstance.ProcessId, out parentProcessId,
                        out parentActivityId, out string parentEspaceKey, out key);
                    ...
                    var currentEspaceKeyAsString = ObjectKeyUtils.DatabaseValue(CurrentEspaceKey);
                    while (hasParent && (parentEspaceKey != currentEspaceKeyAsString)) {
                        GetActivityDataForWS(parentActivityId, out parentProcessId, out ActivityKind kind, out string name, out key, out string url, out int tenantId, out string espaceName,
                            out string processName, out string espaceKey);
                ...
                if (hasParent) {
                    _GetProcessActivityImplementation(parentProcessId, parentActivityId, ObjectKey.Parse(key), false, out IProcessActivity parentActivity);
            ...
            protected virtual void PushDebuggerStack(string processDetail) {
                // Default implementation does not need to do anything, only relevant when Debug enabled.
            }

            protected virtual void SetRequestName(string verb) {
                // Default implementation does not need to do anything, only relevant when Debug enabled.
            }
            ...
            protected Dictionary<ObjectKey, IActivityVariable> ActivityOutputs {
                get {
                    return activityOutputs ??= new Dictionary<ObjectKey, IActivityVariable>();
                }
            }
            ...
            private Dictionary<ObjectKey, IProcessActivity> PrecedentActivitiesCache {
                get {
                    return precedentActivitiesCache ??= new Dictionary<ObjectKey, IProcessActivity>();
                }
            }
            ...
            public T GetPrecedentActivity<T>(ObjectKey targetSSKey) {
                if (PrecedentActivitiesCache.TryGetValue(targetSSKey, out IProcessActivity instance)) {
                    return (T)instance;
            ...
            private IEnumerable<Triplet<int, ActivityKind, ObjectKey>> ExecuteNextActivity(HeContext heContext, Triplet<int, ActivityKind, ObjectKey> nextAct, bool isUnattended) {
                ...
                if (GetProcessActivityInstance(ProcessId, nextAct.First, nextAct.Third, true, out IProcessActivity execObj)) {
            ...
            public bool StartOnOpen(HeContext heContext, string sessionId, bool requestIsSecure, string hostname, bool onlyGetUrl, out string failureMessage, out string handlingUrl) {
                ...
                try {
                    if (onlyGetUrl) {
                        ...
                        StartActivityExecution(heContext, ActivityId, ProcessId, out bool runnable, out ActivityStatus currentStatus);
            ...
            internal static bool CanActivityBeReleased(bool resetActivityUser, ActivityStatus currentStatus, ActivityStatus[] allowedNextStatus) {
                return (resetActivityUser || currentStatus != ActivityStatus.Ready) && allowedNextStatus.Contains(ActivityStatus.Ready);
            }
            ...
            public bool StartActivityReset(HeContext heContext, int activityId, bool resetActivityUser, out string failureMessage) {
                ...
                try {
                    ...
                    if (runnable) {
                        ...
                        AddHandleBreakPoint(currentStatus);
                        List<ActivityStatus> allowedNextStatus = GetNextStatus(currentStatus);
                        if (CanActivityBeReleased(resetActivityUser, currentStatus, allowedNextStatus.ToArray())) {
            ...
            protected void SetCloseInputs(Dictionary<ObjectKey, object> inputValues) {
                ...
                foreach (var kvp in inputValues) {
                    if (ActivityOutputs.TryGetValue(kvp.Key, out IActivityVariable output)) {
            ...
            protected IEnumerable<Triplet<int, ActivityKind, ObjectKey>> CloseActivity(HeContext heContext) {
                ...
                if (CloseActivityInstance(heContext, ActivityId, OnCloseTerminatesProcess, out bool processEnded)) {
            ...
            protected virtual void UpdateProcessInboxDetail(HeContext heContext) {
                try {
                    ...
                    Log.ErrorLog.LogApplicationError(e, heContext.AppInfo, heContext, "BPM");
                }
            }
            ...
            public IEnumerable<Triplet<int, ActivityKind, ObjectKey>> StartWork(HeContext heContext, bool isUnattended, ActivityStatus forInitialStatus, DateTime previousNextRun) {
                ...
                try {
                    ...
                    StartActivityExecution(heContext, ActivityId, ProcessId, forInitialStatus, out bool runnable, out initialStatus, previousNextRun);
