...
namespace OutSystems.HubEdition.RuntimePlatform.Processes {
    public interface IBPMRuntime {
        void GetActivityDataForWS(int activityId, out int processId, out string activityDefinitionKey, out string url, out int tenantId);
    }

    public abstract class BPMRuntime : IBPMRuntime {
        ...
        public void GetActivityDataForWS(int activityId, out int processId, out string activityDefinitionKey, out string url, out int tenantId) {
            GetActivityDataForWS(activityId, out processId, out _, out _, out activityDefinitionKey, out url, out tenantId, out _, out _, out _);
        }
        ...
        protected bool CloseActivityInstance(HeContext heContext, int activityId, bool terminateProcess, out bool processEnded) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction())
            {
                ...
                OSTrace.Debug($"[BPMRuntime > CloseActivityInstance] tenantId = {tenantId}; processId = {processId}; activityId = {activityId}; currentStatus = {currentStatus}, terminateProcess = {terminateProcess}; processEnded = {processEnded}; Kind.IsTerminal = {Kind.IsTerminal()};");
                ...
                OSTrace.Debug($"[BPMRuntime > CloseActivityInstance] tenantId = {tenantId}; processId = {processId}; activityId = {activityId}; currentStatus = {currentStatus}, terminateProcess = {terminateProcess}; processEnded = {processEnded}; Kind.IsTerminal = {Kind.IsTerminal()};" +
                    $" notAlreadyClosed = {notAlreadyClosed};");

                return notAlreadyClosed;
            }
        }

        private static void GetProcessForUpdateAndGetNonTerminatedActivities(Transaction trans, int tenantId, int processId, out int parentActivityId, out ProcessStatus processStatus, out List<Pair<int, ObjectKey>> nonTerminatedActivities)
        {
            ...
            {
                List<Pair<int, ObjectKey>> nonTerminatedActivitiesAux = new List<Pair<int, ObjectKey>>(); 
                while (reader.Read())
                {
                    nonTerminatedActivitiesAux.Add(Pair.Create(reader.SafeGet<int>("ID"), ObjectKey.Parse(reader.SafeGet<string>("SS_Key"))));
                }
                nonTerminatedActivities = nonTerminatedActivitiesAux;
            }
        }
        ...
        /// <param name="processDefinition">Instance of the process to close, can be a dummy instance</param>
        /// <param name="processId">Id of the process, cannot be dummy</param>
        /// <param name="manualTerminate"></param>
        private static void CloseProcessInstance(HeContext heContext, IProcess processDefinition, int processId, bool manualTerminate) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                int tenantId = heContext.AppInfo.Tenant.Id;
                List<Pair<int, ObjectKey>> nonTerminatedActivities;
                int userId = manualTerminate ? heContext.Session.UserId : 0;
                int parentActivityId = 0;
                ProcessStatus processStatus;

                GetProcessForUpdateAndGetNonTerminatedActivities(trans, tenantId, processId, out parentActivityId, out processStatus, out nonTerminatedActivities);

                OSTrace.Debug($"[BPMRuntime > CloseProcessInstance] tenantId = {tenantId}; processId = {processId}; processStatus = {processStatus}; parentActivityId = {parentActivityId}; processDefinition = {processDefinition}; manualTerminate = {manualTerminate};");

                foreach (var nonTerminatedActivity in nonTerminatedActivities) {
                    ActivityStatus activityStatus = ActivityStatus.Closed;

                    //Change all the pending activity status WITHOUT any commits
                    InnerSetActivityStatus(heContext, processId, nonTerminatedActivity.First, activityStatus, manualTerminate ? heContext.Session.UserId : userId);

                    if (processDefinition.GetProcessActivityInstance(processId, nonTerminatedActivity.First, nonTerminatedActivity.Second, false, out IProcessActivity activityToClose)) {
                    ...
                    OSTrace.Debug($"[BPMRuntime > CloseProcessInstance] tenantId = {tenantId}; processId = {processId}; processStatus = {processStatus}; parentActivityId = {parentActivityId}; processDefinition = {processDefinition}; manualTerminate = {manualTerminate};" +
                        $" nonTerminatedActivity.First = {nonTerminatedActivity.First}; ActivityStatus = {activityStatus};");
                }
                ...
                if (parentActivityId != 0) {
                    DBRuntimePlatform.Instance.GetParentProcessId(trans, parentActivityId, out int parentTenantId);
                ...
                DBRuntimePlatform.Instance.SetProcessStatus(trans, tenantId, processId, processStatus, userId);
        ...
        private static void TerminateProcessInstance(HeContext heContext, IProcess processDefinition, int processId, bool recursive) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                int tenantId = heContext.AppInfo.Tenant.Id;
                int parentActivityId = 0;
                List<Pair<int, ObjectKey>> nonTerminatedActivities;
                ...
                OSTrace.Debug($"[BPMRuntime > TerminateProcessInstance] tenantId = {tenantId}; processId = {processId}; processStatus = {processStatus}; parentActivityId = {parentActivityId}; processDefinition = {processDefinition}; recursive = {recursive}");
                ...
                foreach (var nonTerminatedActivity in nonTerminatedActivities) {
                    ...
                    OSTrace.Debug($"[BPMRuntime > CloseProcessInstance] tenantId = {tenantId}; processId = {processId}; processStatus = {processStatus}; parentActivityId = {parentActivityId}; processDefinition = {processDefinition}; recursive = {recursive};" +
                        $" nonTerminatedActivity.First = {nonTerminatedActivity.First}; ActivityStatus = {ActivityStatus.Terminated};");
                }
                ...
                DBRuntimePlatform.Instance.SetProcessStatus(trans, tenantId, processId, processStatus, heContext.Session.UserId);
            ...
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                using (IDataReader reader = DBRuntimePlatform.Instance.GetActivity(trans, tenantId, activityId, true)) {
                    if (reader.Read()) {
                        ...
                        nextRun = reader.SafeGet<DateTime>("NEXT_RUN");
                        currentStatus = (ActivityStatus)reader.SafeGet<int>("STATUS_ID");

                        isRunningSince = reader.SafeGet<DateTime>("IS_RUNNING_SINCE", BuiltInFunction.NullDate());
                ...
                OSTrace.Debug($"[BPMRuntime > StartActivityExecution] tenantId = {tenantId}; nextRun = {nextRun}; currentStatus = {currentStatus}; isRunningSince = {isRunningSince}; runnable = {runnable}; activityId = {activityId}; processId = {processId}; forInitialStatus = {forInitialStatus}; previousNextRun = {previousNextRun}");
        ...
        protected void EndActivityExecution(HeContext heContext, int activityId, bool reschedule, DateTime? explicitNextRun, bool hasError, string errorId) {
            ...
            OSTrace.Debug($"[BPMRuntime > EndActivityExecution] tenantId = {tenantId}; activityId = {activityId}; reschedule = {reschedule}; explicitNextRun = {explicitNextRun}; hasError  = {hasError}; errorId = {errorId}");

            Debugger.CleanCurrentProcessInfo(false);

            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                DBRuntimePlatform.Instance.UpdateActivity(trans, tenantId, activityId, null, null, null, null, null, null, null, null,
                    false, string.Empty, hasError, errorId, reschedule ? (explicitNextRun == null) : (bool?)null, explicitNextRun, null, null, null, null, null);
        ...
        protected static void InnerSetActivityStatus(HeContext heContext, int processId, int activityId, ActivityStatus status, int userIdForLog) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction())
            {
                ...
                OSTrace.Debug($"[BPMRuntime > InnerSetActivityStatus] tenantId = {tenantId}; processId = {processId}; activityId = {activityId}; status = {status}; userIdForLog  = {userIdForLog}");
        ...
        public static int CreateActivityInstance(HeContext heContext, ObjectKey espaceSSKey, ObjectKey activitySSKey, int precedentActivityId, string precedentOutcome) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                return CreateActivityInstance(trans, tenantId, activityDefId, processId, activityDefName, precedentActivityId, precedentOutcome);
            }
        }
        ...
        public static int CreateProcessInstance(HeContext heContext, int tenantId, ObjectKey espaceSSKey, ObjectKey processSSKey, int parentActivityId, int parentProcessId, out Pair<int, ObjectKey> startActivity) {
            ...
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                if (parentActivityId != BuiltInFunction.NullIdentifier()) {
                    parentProcessId = DBRuntimePlatform.Instance.GetParentProcessId(trans, parentActivityId, out int parentTenantId);
                }
                ...
                OSTrace.Debug($"[BPMRuntime > CreateProcessInstance] tenantId = {tenantId}; createdBy = {createdBy}; processDefId = {processDefId}; parentProcessId = {parentProcessId}; parentActivityId  = {parentActivityId}; topProcessId = {topProcessId};" +
                    $" espaceSSKey = {espaceSSKey}; processSSKey = {processSSKey}");
                ...
                startActivityId = CreateActivityInstance(trans, tenantId, activityDefId, newProcessId, activityDefName, 0, null);
            }
        ...
        protected void ReleaseActivity(HeContext heContext, int activityId, bool resetActivityUser) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                OSTrace.Debug($"[BPMRuntime > ReleaseActivity] tenantId = {heContext.AppInfo.Tenant.Id}; activityId = {activityId}; activityDefId = {activityDefId}; activityDefKind = {activityDefKind}; resetActivityUser = {resetActivityUser}");
        ...
        protected void ScheduleActivity(HeContext heContext, int activityId, DateTime startDate) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                OSTrace.Debug($"[BPMRuntime > ScheduleActivity] tenantId = {heContext.AppInfo.Tenant.Id}; activityId = {activityId}; activityDefId = {activityDefId}; activityDefKind = {activityDefKind}; startDate = {startDate}");
        ...
        public static void TerminateProcessInstance(HeContext heContext, IProcess processDefinition, int processId) {
            ...
            OSTrace.Debug($"[BPMRuntime > TerminateProcessInstance] tenantId = {heContext.AppInfo.Tenant.Id}; processDefinition = {processDefinition}; processId  = {processId}; currentStatus = {currentStatus}");
        }
        ...
        public static void SuspendProcessInstance(HeContext heContext, int processId, bool suspend) {
            ...
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                OSTrace.Debug($"[BPMRuntime > SuspendProcessInstance] tenantId = {heContext.AppInfo.Tenant.Id}; processId = {processId}; currentStatus  = {currentStatus}; suspend = {suspend}");
        ...
        public static void GetActivityDataForWS(int activityId, out int processId, out ActivityKind activityKind, out string activityName, out string SSKey, out string url, out int tenantId, out string espaceKey) {
            GetActivityDataForWS(activityId, out processId, out activityKind, out activityName, out SSKey, out url, out tenantId, out _, out _, out espaceKey);
        }
            ...
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                using (IDataReader reader = DBRuntimePlatform.Instance.GetActivityDataForWS(trans, activityId)) {
                    if (reader.Read()) {
                        ...
                        SSKey = reader.SafeGet<string>("SS_Key");
                        tenantId = reader.SafeGet<int>("Tenant_Id");
                        processId = reader.SafeGet<int>("Process_Id");
                        activityKind = (ActivityKind)reader.SafeGet<int>("ActivityKind");
                        activityName = reader.SafeGet<string>("ActivityName");
                        processName = reader.SafeGet<string>("ProcessName");
                        espaceKey = reader.SafeGet<string>("ESPACE_KEY");
                    } else {
        ...
        public static void GetProcessDataForWS(int processId, out string SSKey, out string url, out int tenantId, out string eSpaceKey) {
            GetProcessDataForWS(processId, out SSKey, out url, out tenantId, out eSpaceKey, out _);
        }

        public static void GetProcessDataForWS(int processId, out string SSKey, out string url, out int tenantId, out string eSpaceKey, out bool isActive) {
            string EspaceName;
        ...
        public static string GetUrlForWS(ObjectKey eSpaceKey, string eSpaceName) {
        ...
        public static IEnumerable<int> GetAllowedActivities(HeContext heContext, Transaction tran, IEnumerable<int> activityIds, int userId) {
            foreach (var nextId in activityIds) {
                bool needsPermission = DBRuntimePlatform.Instance.NeedsActivityPermissions(tran, nextId, out int assignedUser, out ActivityStatus statusId);
                ...
                if (assignedUser != BuiltInFunction.NullIdentifier()) {
                    ...
                    if (needsPermission) {
                        ...
                        if (!roles.Any(role => GenericExtendedActions.Check(heContext, role.RoleKey, role.EspaceKey, userId))) {
                            continue;
                        }
                    }
                }
        ...
        // private actions
        private static int CreateActivityInstance(Transaction tran, int tenantId, int activityDefId, int processId,
            ...
            OSTrace.Debug($"[BPMRuntime > CreateActivityInstance] tenantId = {tenantId}; activityDefId = {activityDefId}; processId = {processId}; activityDefName = {activityDefName}; precedentActivityId = {precedentActivityId};" +
                $" precedentOutcome = {precedentOutcome}; createdActivityId = {createdActivityId}");

            return createdActivityId;
        }

        public static void FlushPendingActivityEvent(int activityId, out int processedEvents) {
            FlushPendingActivityEvent(AppInfo.GetAppInfo()?.OsContext, activityId, null, out processedEvents);
        }
        ...
        public static void FlushPendingActivityEvent(HeContext heContext, int activityId, Func<string, ActivitiesClientFactory> getFactory, out int processedEvents) {
            if (activityId == 0) {
                processedEvents = 0;
                return;
            }
            ...
            try {
                BPMRuntime.GetActivityDataForWS(eActivityId, out int processId, out ActivityKind activityKind, out string activityName, out string SSKey, out string url, out int tenantId, out string espaceKey);
                ...
                IActivitiesClient GetClient() {
                    if (getFactory != null) {
                        return getFactory(url).CreateClient();
                    } else {
                        return new ActivitiesClientFactory(RuntimeSettingsProvider.Instance, url, tenantId, heContext?.Session?.UserId ?? 0, heContext?.AppInfo?.eSpaceUID ?? string.Empty, espaceKey).CreateClient();
                    }
                }

                using (IActivitiesClient activityHandler = GetClient()) {
                    activityHandler.Timeout = TimeSpan.FromSeconds((int)(1.2 * timeoutInSecs));
                ...
                OSTrace.Debug($"[BPMRuntime > FlushPendingActivityEvent] tenantId = {tenantId}; processId = {processId}; eId = {eId}; eActivityId = {eActivityId}; eDataId = {eDataId}; SSKey = {SSKey}; processedEvents = {processedEvents};");
            }
            catch (Exception e) {
                using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetCommitableTransaction()) {
                ...
                OSTrace.Debug($"[BPMRuntime > FlushPendingActivityEvent] eId = {eId}; eActivityId = {eActivityId}; eDataId = {eDataId}; stackTrace = {e.StackTrace}; processedEvents = {processedEvents}");
            }
        }

        public static void ValidateToken(HeContext heContext, int userId, int tenantId, string eSpaceKey) {

            if (!heContext.CurrentHttpContext.Request.Headers.TryGetValue("Authorization", out string authorizationHeaderValue)) {
                // check if in compatibility mode with pre-O11 versions
                if (RuntimeSettingsProvider.Instance.Get(RuntimePlatformSettings.Authentication.ReferencesAllowLegacyAuth)) {
                    // Check if the request is from a valid IP
                    if (!NetworkInterfaceUtils.IsLoopbackAddress(heContext.CurrentHttpContext.Request.UserHostAddress)) {
                        var exception = new Exception("The activity actions handler can only be accessed by the 127.0.0.1 IP");
                        ErrorLog.LogApplicationError(exception, heContext.AppInfo, heContext, "BPM");
