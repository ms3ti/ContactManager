...
namespace OutSystems.HubEdition.RuntimePlatform.Processes {
    public abstract partial class ProcessBase : IProcess {
        ...
        protected static ActivityVariable<T> GetVariable<T>(Dictionary<ObjectKey, IActivityVariable> collection, ActivityVariable<T> variableDefinition, GetVariablesDelegate getMethod, int id) {
            ...
            if (collection.TryGetValue(variableDefinition.Key, out IActivityVariable variable)) {
        ...
        protected Dictionary<ObjectKey, IActivityVariable> ProcessInputs {
            get {
                return processInputs ??= new Dictionary<ObjectKey, IActivityVariable>();
            }
        }
        ...
        public Dictionary<ObjectKey, IActivityVariable> ProcessOutputs {
            get {
                return processOutputs ??= new Dictionary<ObjectKey, IActivityVariable>();
            }
            set {
        ...
        public void SaveUnsetProcessOutputs(HeContext heContext) {
            using (Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
                ...
                //this local variable is necessary since the Property initializes an inner class
                //with static variables that execute Queries. If the Query of the inner class static property
                //is executed inside the reader, in some Database Stacks will thrown an error since Multiple Active
                //Result Sets might not be supported.
                var processOutputDefinitions = AllProcessOutputsDefinitions;
                ...
                using (IDataReader reader = DBRuntimePlatform.Instance.GetUnsetProcessOutputs(trans, ProcessId)) {
                    while (reader.Read()) {
                        ...
                        if (processOutputDefinitions.TryGetValue(ObjectKey.Parse(sskey), out IActivityVariable outputDef)) {
        ...
        public int ProcessLaunch(HeContext heContext, int parentActivityId, int parentProcessId, List<Pair<string, object>> inputs, out List<Pair<int, ActivityKind>> nextActIds) {
            ...
            int tenantId = heContext.AppInfo.Tenant.Id;
            processIdOverride = BPMRuntime.CreateProcessInstance(heContext, tenantId, GlobalKey.OwnerKey, GlobalKey.Key, parentActivityId, parentProcessId, out Pair<int, ObjectKey> startActivity);
            ...
            foreach (Pair<string, object> input in inputs) {
                ...
                if (inputDefinitions.TryGetValue(key, out IActivityVariable curInput)) {
            ...
            DatabaseAccess.CommitAllTransactions();
                try {
                    if (GetProcessActivityInstance(ProcessId, startActivity.First, startActivity.Second, true, out IProcessActivity startActivityInstance)) {
        ...
        public static string ProcessException(HeContext heContext, Exception e, string extraMessage, bool skipLog) {
            return ProcessException(heContext, e, string.Empty, skipLog, out string errorId);
        }
        ...
        public static string ProcessException(HeContext heContext, Exception e, string extraMessage, bool skipLog, out string errorId) {
            DatabaseAccess.RollbackAllTransactions();
            int tenantId = heContext.AppInfo.Tenant.Id;
            int eSpaceId = heContext.AppInfo.eSpaceId;
            ...
            if (!skipLog) {
                if (e is AbortActivityChangeException) {
                    if (heContext.AppInfo.Properties.AllowAuditing) {
                        Log.GeneralLog.StaticWrite(DateTime.Now, "", eSpaceId, tenantId,
                        userId, e.Message, "USER", "AbortActivity", "");
                    }
                } else {
                    errorId = Log.ErrorLog.LogApplicationError(e, heContext.AppInfo, heContext, "BPM");
                }
            }
            ...
            OSTrace.Debug($"[ProcessBase > ProcessException] tenantId = {tenantId}; eSpaceId = {eSpaceId}; userId= {userId}; message = {e.Message}; extraMessage = {extraMessage}; skipLog = {skipLog}; errorId = {errorId}; stackTrace = {e.ToString()}");
