...
namespace OutSystems.HubEdition.DatabaseProvider.iDB2.ExecutionService {
    ...
    static class ParametersHandler {
        ...
        private const int MAX_VARCHAR_SIZE = 32740;

        private const string DISABLE_DB2_QUERYPARAMETERCASTS = "OutSystems.DatabaseProvider.iDB2.DisableDb2QueryParameterCasts";

        private static bool AppendCast {
            get {
                bool.TryParse(ConfigurationManager.AppSettings[DISABLE_DB2_QUERYPARAMETERCASTS], out bool parameterValue);
        ...
        public static IDbCommand CastAndMultiplicateParameters(IDbCommand cmd) {
            ...
            //this happens when the param is at the end of the string, if so, we need to close the cast
            if (hasCasted && AppendCast) {
                string paramName = query.Substring(currParamBeginIndex, i - currParamBeginIndex);
                appendCastEnd(parametersDictionary, paramName, sb);
                hasCasted = false;
                hitSeparatorChar = false;
