...
namespace OutSystems.HubEdition.DatabaseProvider.SqlServer.DatabaseObjects {
    public class DatabaseObjectFactory : IDatabaseObjectFactory {
        ...
        public IDatabaseInfo CreateDatabaseInfo(string databaseIdentifier) {
            if (ParseDatabaseIdentifier(databaseIdentifier, out string linkedServer, out string catalog)) {
        ...
        public virtual ITableSourceInfo CreateTableSourceInfo(string qualifiedName) {
            ...
            if (ParseQualifiedTableName(qualifiedName, out string linkedServer, out string catalog, out string schema, out string table, out bool unableToCheckForDBLink)) {
        ...
        private bool ParseDatabaseIdentifier(string databaseIdentifier, out string linkedServer, out string catalog) {
            linkedServer = string.Empty;
            catalog = string.Empty;
            MatchCollection databaseIdentifierParts = regexCaptureIdentifiers.Matches(databaseIdentifier);
            if (databaseIdentifierParts.Count == 0 || databaseIdentifierParts.Count > 2) {
                return false;
            }

            for (int i = databaseIdentifierParts.Count - 1; i >= 0; i--) {
                string part = databaseIdentifierParts[i].Value.Trim(trimChars);
        ...
        private bool ParseQualifiedTableName(string qualifiedName, out string linkedServer, out string catalog,
                                             out string schema, out string table, out bool unableToCheckForDBLink) {
            linkedServer = string.Empty;
            catalog = string.Empty;
            schema = string.Empty;
            table = string.Empty;
            unableToCheckForDBLink = false;
            ...
            // No server name in the name parts, we may be introspecting a view that has a definition based on a link
            if (serverToTest.IsNullOrEmpty()
                && ProviderSettings.AllowViewsDBLinkIntrospection(databaseServices)) {

                TryIntrospectingViewForPotentialDBLink(ref unableToCheckForDBLink, ref tableNameParts);
            }
        ...
        private void TryIntrospectingViewForPotentialDBLink(ref bool unableToCheckForDBLink, ref MatchCollection tableNameParts) {
            // #840789 - When publishing a reference to an extension, it might not be possible to connect to the database
            bool isDatabaseReachable = databaseServices.TransactionService.TestConnection(out _);

            if (!isDatabaseReachable) {
                unableToCheckForDBLink = true;
            } else {
                ...
                string escapedCatalog = DMLIdentifiers.EscapeIdentifierInner(currentCatalog);
                string sql = $@"SELECT VIEW_DEFINITION
                                FROM {escapedCatalog}.INFORMATION_SCHEMA.VIEWS
                ...
                while (reader.Read()) {
                    ...
                    // If the user doing the instrospection is missing the VIEW ALL DEFINITION permission
                    // We will not be able to know if the view is based on a DB Link
                    // The result will be the same as treating it as a local view without DB Link
                    ...
                    const string RegexPart1 = @"([\[""][^""\]\[]+?[\]""]|(?<![""\[][^\s]*)[^\.\[""\s/-]+)(?![""\]])";
                    const string RegexPart234 = @"([\[""][^""\]\[]+?[\]""]|(?<![""\[])[^\.\[""\s/-]+)(?![""\]])";
                    Regex regexCaptureFourPartIdentifiers = new Regex(RegexPart1 + @"(\." + RegexPart234 + "){3}", RegexOptions.Compiled);
