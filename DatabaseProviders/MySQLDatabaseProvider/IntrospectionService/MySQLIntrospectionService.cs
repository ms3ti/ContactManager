...
namespace OutSystems.HubEdition.DatabaseProvider.MySQL.IntrospectionService {
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Minor Code Smell", "S101:Types should be named in PascalCase", Justification = "Classname represents provider name")]
    public class MySQLIntrospectionService : BaseIntrospectionService, IIntrospectionServiceWithPagination {
        ...
        public override IEnumerable<ITableSourceInfo> ListTableSources(IDatabaseInfo database, IsTableSourceToIgnore isTableSourceToIgnore) {
            ...
            MySQLDatabaseInfo databaseInfo = (MySQLDatabaseInfo)database;
        ...
        public IEnumerable<ITableSourceInfo> ListTableSources(IDatabaseInfo database, IsTableSourceToIgnore isTableSourceToIgnore, int limit, int offset, out int total) {
            if (!(database is MySQLDatabaseInfo databaseInfo)) {
                throw new IntrospectionServiceException("Expected " + typeof(MySQLDatabaseInfo).FullName + " type but found " +
        ...
        private IEnumerable<ITableSourceInfo> GetListTableSourceInfos(MySQLDatabaseInfo databaseInfo, IDbConnection connection, IsTableSourceToIgnore isTableSourceToIgnore, int limit, int offset) {
            IList<ITableSourceInfo> tableSourceInfos = new List<ITableSourceInfo>();
            string paramPrefix = DatabaseServices.ExecutionService.ParameterPrefix;
            string sql = $@"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = {paramPrefix + "schema"}
                            UNION
                            SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = {paramPrefix + "schema"}
                            ORDER BY TABLE_NAME
                            LIMIT {paramPrefix + "limit"} OFFSET {paramPrefix + "offset"};";
            ...
            while (reader.Read()) {
                ...
                if (!isTableSourceToIgnore(tableName)) {
                    string qualifiedTableName = GetQualifiedIdentifier(databaseInfo.Name, tableName);
                    tableSourceInfos.Add(new MySQLTableSourceInfo(DatabaseServices, databaseInfo, tableName, qualifiedTableName));
                }
            }
        ...
        private int GetTableSourcesCount(MySQLDatabaseInfo databaseInfo) {
            string paramPrefix = DatabaseServices.ExecutionService.ParameterPrefix;
            string sql = $@"SELECT COUNT(*)
                            FROM (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = {paramPrefix + "schema"}
                                  UNION
                                  SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = {paramPrefix + "schema"}) 
                            AS TABLE_NAMES;";

            using IDbConnection connnection = DatabaseServices.TransactionService.CreateConnection();
            using IDbCommand cmd = DatabaseServices.ExecutionService.CreateCommand(connnection, sql);
            cmd.CommandTimeout = QueryTimeout;
            DatabaseServices.ExecutionService.CreateParameter(cmd, paramPrefix + "schema", DbType.String, databaseInfo.Name);
            ...
            using (IDbConnection conn = DatabaseServices.TransactionService.CreateConnection()) {
                ...
                using (IDataReader reader = cmd.ExecuteReader()) {
                    while (reader.Read()) {
                        ...
                        Int32.TryParse(Convert.ToString(reader["CHARACTER_MAXIMUM_LENGTH"]), out int length);
                        ...
                        if (pieces.Length < 2 || int.TryParse(pieces[1], out int precision) == false) {
