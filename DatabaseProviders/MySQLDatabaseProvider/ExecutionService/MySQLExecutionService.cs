...
namespace OutSystems.HubEdition.DatabaseProvider.MySQL.ExecutionService {
    ...
    public class MySQLExecutionService : BaseExecutionService {
        ...
        // TODO Due to RPD-4718 originated from a MySQL bug (https://bugs.mysql.com/bug.php?id=89159), we need to encapsulate
        // commands to avoid executing the MySQL connector's Dispose command. When a fix is released, we should revert this change
        private class MySQLCommand : IDbCommand {
            ...
            public MySQLCommand(IDbCommand dbCommand) {
        ...
        private readonly bool useMySqlDriver18Fix;

        public MySQLExecutionService(IDatabaseServices databaseServices) : base(databaseServices) {
            useMySqlDriver18Fix = MySQLProviderSettings.UseMySqlDriver18Fix;
            DatabasePropertiesService = new MySQLDatabasePropertiesService(databaseServices);
        }
        ...
        protected override IDbCommand CreateCommand(IDbConnection connection) {
            if (useMySqlDriver18Fix) {
                return new MySQLCommand(base.CreateCommand(connection));
            } else {
