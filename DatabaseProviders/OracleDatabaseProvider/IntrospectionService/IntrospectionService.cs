...
namespace OutSystems.HubEdition.DatabaseProvider.Oracle.IntrospectionService {
    ...
    public class IntrospectionService : BaseIntrospectionService, IIntrospectionServiceWithPagination {
        ...
        public override IEnumerable<ITableSourceInfo> ListTableSources(IDatabaseInfo database, IsTableSourceToIgnore isTableSourceToIgnore) {
            DatabaseInfo databaseInfo = (DatabaseInfo)database;
            using (IDbConnection conn = DatabaseServices.TransactionService.CreateConnection()) {
        ...
        public IEnumerable<ITableSourceInfo> ListTableSources(IDatabaseInfo database, IsTableSourceToIgnore isTableSourceToIgnore, int limit, int offset, out int total) {
            if (!(database is DatabaseInfo databaseInfo)) {
            ...
            total = GetTableSourcesCount(databaseInfo);

            using IDbConnection connection = DatabaseServices.TransactionService.CreateConnection();
            if (!OwnerExists(connection, databaseInfo)) {
                throw new IntrospectionServiceException("Inexistent database: " + databaseInfo.Schema);
            }

            return GetListTableSourceInfos(databaseInfo, connection, isTableSourceToIgnore, limit, offset);
        }

        private IEnumerable<ITableSourceInfo> GetListTableSourceInfos(DatabaseInfo databaseInfo, IDbConnection connection, IsTableSourceToIgnore isTableSourceToIgnore, int limit, int offset) {
            IList<ITableSourceInfo> tableSourceInfos = new List<ITableSourceInfo>();
            string link = GetDatabaseLinkName(databaseInfo.DatabaseLink, databaseInfo.IsDatabaseLink);
            string paramPrefix = DatabaseServices.ExecutionService.ParameterPrefix;
            string sql = $@"select OBJECT_NAME TABLE_NAME from all_objects {link}
                            where not (OBJECT_NAME like 'BIN$%$%') and (OBJECT_TYPE = 'TABLE' or OBJECT_TYPE = 'VIEW') and owner = {paramPrefix + "dbName"}
                            order by TABLE_NAME
                            offset {paramPrefix + "offset"} rows
                            fetch first {paramPrefix + "limit"} rows only";

            using IDbCommand cmd = DatabaseServices.ExecutionService.CreateCommand(connection, sql);
            cmd.CommandTimeout = QueryTimeout;
            DatabaseServices.ExecutionService.CreateParameter(cmd, paramPrefix + "offset", DbType.Int32, offset);
            DatabaseServices.ExecutionService.CreateParameter(cmd, paramPrefix + "limit", DbType.Int32, limit);
            DatabaseServices.ExecutionService.CreateParameter(cmd, paramPrefix + "dbName", DbType.String, databaseInfo.Schema);
            ...
            while (reader.Read()) {
                ...
                tableSourceInfos.Add(new TableSourceInfo(DatabaseServices, databaseInfo, tableName, GetQualifiedTableName(databaseInfo, tableName)));
            }

            return tableSourceInfos;
        }

        private int GetTableSourcesCount(DatabaseInfo databaseInfo) {
            using IDbConnection conn = DatabaseServices.TransactionService.CreateConnection();
            if (!OwnerExists(conn, databaseInfo)) {
                throw new IntrospectionServiceException("Inexistent database: " + databaseInfo.Schema);
            }
            ...
            string link = GetDatabaseLinkName(databaseInfo.DatabaseLink, databaseInfo.IsDatabaseLink);
            string paramPrefix = DatabaseServices.ExecutionService.ParameterPrefix;
            string sql = $@"select count(*) from all_objects {link}
                         where not (OBJECT_NAME like 'BIN$%$%') and (OBJECT_TYPE = 'TABLE' or OBJECT_TYPE = 'VIEW') and owner = {paramPrefix + "dbName"}";
            ...
            using (IDbConnection conn = DatabaseServices.TransactionService.CreateConnection()) {
                ...
                using (IDataReader reader = GetColumnsBulk(conn, tableSourceList, dbInfo)) {
                    while (reader.Read()) {
                        ...
                        autoGeneratedColumns.TryGetValue(GetAutoNumberColumnKey(tableName, columnName), out AutoNumberColumnInfo autoNumberInfo);
                        if (autoNumberInfo != null && !(dataDefault.IsNullOrEmpty() || dataDefault.ToUpper() == "NULL")) {
                            // trigger AlterColumn on an autonumber column that had been incorrectly created (RPM-882)
                            precision -= 1;
                        }
                        ...
                        if (autoNumberInfo == null && IsSequenceValue(dataDefault)) { // For versions above 12c (12.1) autonumbers can be generated via identity or sequences
        ...
        protected IDataReader GetColumnsBulk(IDbConnection conn, IEnumerable<TableSourceInfo> tableSources, DatabaseInfo dbInfo) {
            ...
            string tableCondition = GenerateTableCondition("atc.table_name", tableSources, out string conditionParam, out string conditionValue);
        ...
        protected IDataReader GetTriggersBulk(IDbConnection conn, IEnumerable<TableSourceInfo> tableSources, bool onlyAutoNumbers, string filterByPrefix,
            ...
            string tableCondition = GenerateTableCondition("table_name", tableSources, out string conditionParam, out string conditionValue);
        ...
        protected IDataReader GetForeignKeysBulk(IDbConnection conn, IEnumerable<TableSourceInfo> tableSources, DatabaseInfo dbInfo) {
            ...
            string tableCondition = GenerateTableCondition("c.table_name", tableSources, out string conditionParam, out string conditionValue);
        ...
        private string GenerateTableCondition(string columnName, IEnumerable<TableSourceInfo> tableSourceList, out string conditionParameter, out string conditionValue) {
            return GenerateTableCondition(columnName, tableSourceList.Select(ts => ts.Name), out conditionParameter, out conditionValue);
        }

        internal protected string GenerateTableCondition(string columnName, IEnumerable<string> tableSources, out string conditionParameter, out string conditionValue) {
            conditionValue = null;
            string[] tableSourceList = tableSources.ToArray();

            if (tableSourceList.IsSingle()) {
                conditionValue = tableSourceList.First();
                conditionParameter = "TABLENAME";
                return $"{columnName} = {DatabaseServices.ExecutionService.ParameterPrefix}{conditionParameter}";
            } else {
                // #RSAT-1715 - IN Clauses in oracle have a max of 1000 elements, in order to overcome this limitation, we split query with several OR conditions
                conditionParameter = null;
                return DatabaseServices.DMLService.GenerateInStatement(tableSources, columnName);
            }
        }
