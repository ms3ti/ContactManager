...
namespace OutSystems.HubEdition.DatabaseProvider.Oracle.ExecutionService {
    ...
    public class ExecutionService : BaseExecutionService {
        ...
        public ExecutionService(IDatabaseServices databaseServices) : base(databaseServices) {
            DatabasePropertiesService = new OracleDatabasePropertiesService(databaseServices);
        }
        ...
        protected virtual void SetDataReaderFetchSize(OracleDataReader reader) {
            reader.FetchSize *= FETCH_SIZE_INCREASE_FACTOR;
        }
            ...
            try {
                ...
                using (OracleCommand cmd = (OracleCommand)con.CreateCommand()) {
                    ...
                    foreach (DataColumn col in tab.Columns) {
                        ...
                        OracleDbType? oracleType = ConvertIfDoesNotHaveDbType(col.DataType);

                        object[] paramValues;
                        if (oracleType != null) {
                            paramValues = GetParamValues(tab, col, rowsWithoutErrors);
                        } else {
                            DbType dbType = ConvertToDbType(col.DataType);
                            (paramValues, oracleType) = GetParamValuesWithDbType(tab, col, dbType, rowsWithoutErrors);
                        }
                        
                        cmd.Parameters.Add(col.ColumnName, oracleType.Value, paramValues, ParameterDirection.Input);
                    }
        ...
        private (object[], OracleDbType) GetParamValuesWithDbType(DataTable tab, DataColumn col, DbType dbType, 
                                                  int rowsWithoutErrors) {
            ...
            for (int tabIndex = 0; tabIndex < tab.Rows.Count; tabIndex++) {
                if (string.IsNullOrEmpty(tab.Rows[tabIndex].RowError)) {
                    ...
                    string strValue = paramValues[paramIndex] as string;
            ...
            var oracleDbType = ConvertDbTypeToOracleDbType(dbType, maxLength);

            return (paramValues, oracleDbType);
        }
        
        private object[] GetParamValues(DataTable tab, DataColumn col, int rowsWithoutErrors) {
            object[] paramValues = new object[rowsWithoutErrors];
            int paramIndex = 0;
            for (int tabIndex = 0; tabIndex < tab.Rows.Count; tabIndex++) {
                if (string.IsNullOrEmpty(tab.Rows[tabIndex].RowError)) {
                    paramValues[paramIndex] = tab.Rows[tabIndex][col];
                    paramIndex++;
                }
            }
        ...
        private OracleDbType? ConvertIfDoesNotHaveDbType(Type type) {
            if (type == typeof(byte[])) {
