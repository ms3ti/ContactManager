    ...
    public class PlatformIntrospectionService : Oracle.IntrospectionService.IntrospectionService, IPlatformIntrospectionService {
        ...
        public IDictionary<ITableSourceInfo, IPlatformTableSourceInfo> GetTableSourcesDetails(params ITableSourceInfo[] tables) {
            ...
            if (UseSplitMethod()) {
                ...
                foreach (IPlatformTableSourceColumnInfo column in GetColumns(tableSourceList, dbInfo)) {
                    ...
                    if (!tableSourcesColumns.TryGetValue(tableName, out IList<IPlatformTableSourceColumnInfo> tableColumns)) {
                ...
                foreach (var foreignKey in GetForeignKeys(tableSourceList, dbInfo)) {
                    ...
                    if (!tableSourcesForeignKeys.TryGetValue(tableName, out IList<ITableSourceForeignKeyInfo> tableFKeys)) {
        ...
        private void InitializeIndexes(IDictionary<ITableSourceInfo, IPlatformTableSourceInfo> tablesDetails) {
            ...
            while (reader.Read()) {
                ...
                // #RSAT-2336: fixes an issue that was recreating OS indexes on every publish
                var prefix = indexName.Substring(0, indexName.IndexOf('_') + 1).ToUpper();
                var isPlatformIndex = prefix == PlatformDatabaseObjectConstants.IndexPrefix ||
                    prefix == PlatformDatabaseObjectConstants.ForeignConstPrefix ||
                    prefix == PlatformDatabaseObjectConstants.PrimaryConstPrefix ||
                    prefix == PlatformDatabaseObjectConstants.PrimarySysConstPrefix ||
                    prefix == OraclePlatformDatabaseObjectConstants.PrimaryConstInvariantPrefix ||
                    prefix == OraclePlatformDatabaseObjectConstants.PrimarySysConstInvariantPrefix;
                ...
                if (columnsFromExpression.IsEmpty()) {
                    IPlatformTableSourceColumnInfo column = tableSource.Columns.SingleOrDefault(col => col.Name.EqualsIgnoreCase(columnName));
                    if (column != null) {
                        if (isPlatformIndex) {
                            currentIndex.AddColumn(column, /*isFunctionIndex*/false, "");
                        } else {
                            currentIndex.AddColumn(
                                new PlatformTableSourceColumnInfo(
                                    tableSource: column.TableSource,
                                    name: column.Name,
                                    dataType: column.DataType,
                                    isMandatory: column.IsMandatory,
                                    isPrimaryKey: column.IsPrimaryKey,
                                    isAutoNumber: column.IsAutoGenerated), /*isFunctionIndex*/false, "");
                        }
                    }
                } else {
                    columnsFromExpression.Apply(c => {
                        if (isPlatformIndex) {
                            currentIndex.AddColumn(c, /*isFunctionIndex*/true, expression);
                        } else {
                            currentIndex.AddColumn(
                                new PlatformTableSourceColumnInfo(
                                    ...
                                    isAutoNumber: c.IsAutoGenerated), /*isFunctionIndex*/false, "");
                        }
                    });
        ...
        public IDictionary<string, bool> CheckTableSourcesExist(params string[] tableSourcesNames) {
            ...
            using var conn = databaseServices.TransactionService.CreateConnection();
            string tableCondition = GenerateTableCondition("UPPER(table_name)", tableSourceNamesList, out string conditionParam, out string conditionValue);
            string viewCondition = GenerateTableCondition("UPPER(view_name)", tableSourceNamesList, out _, out _);

            var useAll = !dbInfo.Schema.Equals(DatabaseServices.DatabaseConfiguration.Username, StringComparison.InvariantCultureIgnoreCase);
            var ownerParameter = $"{DatabaseServices.ExecutionService.ParameterPrefix}OWNER";
            var sql = "";
            if(useAll) {
                sql = $@"SELECT table_name FROM all_tables WHERE {tableCondition} AND OWNER = {ownerParameter} 
                         UNION 
                         SELECT view_name FROM all_views WHERE {viewCondition} AND OWNER = {ownerParameter}";
                
            } else {
                sql = $@"SELECT table_name FROM user_tables WHERE {tableCondition}
                         UNION
                         SELECT view_name FROM user_views WHERE {viewCondition}";
            }
            using var cmd = databaseServices.ExecutionService.CreateCommand(conn, sql);
            ...
            if (useAll) {
                databaseServices.ExecutionService.CreateParameter(cmd,
                    ownerParameter, DbType.String, dbInfo.Schema);
            }
        ...
        private int GetTableSize(ITableSourceInfo table) {
            int size;
            try {
                string sql = "select bytes/1000 sizeTable from user_segments where segment_name=:tableId";
                using var conn = DatabaseServices.TransactionService.CreateConnection();
                using var cmd = DatabaseServices.ExecutionService.CreateCommand(conn, sql);
                var oracleParameter = new OracleParameter("tableId", table.Name);
                cmd.Parameters.Add(oracleParameter);
                using var reader = DatabaseServices.ExecutionService.ExecuteReader(cmd);
                reader.Read();
                size = Convert.ToInt32(reader["sizeTable"]);
            } catch (InvalidOperationException) {
                //On empty tables oracle does not store any information about its size
                size = 0;
            }
            return size;
        }
