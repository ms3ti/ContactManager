...
namespace OutSystems.HubEdition.DatabaseProvider.Oracle.Platform.DatabaseObjects {
    public class PlatformTableSourceIndexInfo : IPlatformTableSourceIndexInfo {
        ...
        public void AddColumn(IPlatformTableSourceColumnInfo columnInfo, bool isFunctionIndex, string expression) {
            if (columnInfo != null && !columns.Any(col => StringUtils.EqualsIgnoreCase(col.Name, columnInfo.Name))) {
                columns.Add(columnInfo);
                if (isFunctionIndex) {
                    //generated functional indexes have this format NLS_SORT("COL", "NLS_SORT=BINARY_AI")
                    // but for some reason when we introspect this info from oracle, we get different casing, spacing, and quoting, 
                    // so we make the expression uniform, so when it compares we get same result
                    columnInfo.Expression = expression.ToUpperInvariant().Replace("''", "").Replace(" ", "");
                    functionIndexColumns.Add(columnInfo);
                }
            }
        }

        public string GetFunctionalIndexExpression(IPlatformTableSourceColumnInfo column, IDatabaseServices databaseServices) {
            var dbConfig = databaseServices.DatabaseConfiguration as RuntimeDatabaseConfiguration;
            var identifiers = databaseServices.DMLService.Identifiers;

            return $"NLSSORT({identifiers.EscapeIdentifier(column.Name)}, 'NLS_SORT={dbConfig.NLS_Sort}')";
        }
