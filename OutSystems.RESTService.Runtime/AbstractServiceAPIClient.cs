...
namespace OutSystems.RESTService {
    ...
    public abstract class AbstractServiceAPIClient { // TODO: to be moved on #ABE-823
        ...
        protected HttpWebRequest BuildServiceAPIMethodRequest(HeContext context, string sapimName, object inputs) {
            return BuildServiceAPIMethodRequest<JObject>(context, sapimName, JObject.FromObject(inputs));
        }
        ...
        protected HttpWebRequest BuildServiceAPIMethodRequest<T>(HeContext context, string sapimName, T inputs) {
            OSTrace.Debug("[START] BuildServiceAPIMethodRequest");
            var zoneSettings = DeploymentZoneResolution.ByModuleKey(ProducerModuleKey);
            OSTrace.Debug($"Zone address: {zoneSettings.Address}");
            OSTrace.Debug($"Zone enable https: {zoneSettings.EnableHttps}");
            ...
            OSTrace.Debug($"Service Action endpoint: {sapimEndpoint}");
            ...
            var requestPayload = new CoreServicesApiController.Payload.RequestPayload<T>();
            requestPayload.RequestKey = RuntimePlatformUtils.GetRequestTracer()?.RequestKey ?? Guid.NewGuid().ToString();
            requestPayload.InputParameters = inputs;
            string payloadString = JsonConvert.SerializeObject(requestPayload, Formatting.None, ServiceConfiguration.SerializerSettings);
            TraceSerializerSettings(ServiceConfiguration.SerializerSettings);
            ...
            request.Timeout = RuntimePlatformSettings.ServiceAPIs.RequestTimeoutInSeconds.GetValue() * 1000;
            OSTrace.Debug($"Request timeout: {request.Timeout}ms");
            ...
            OSTrace.Debug("[END] BuildServiceAPIMethodRequest");
            return request;
        }
        ...
        protected static Exception ConvertPayloadToException(FailResponsePayload exception) => exception.Exception.Name switch
        {
            "NotRegisteredException" => new NotRegisteredException(exception.Exception.Message),
            "InvalidLoginException" => new InvalidLoginException(exception.Exception.Message),
            "AbortActivityChangeException" => new AbortActivityChangeException(exception.Exception.Message),
            "SecurityException" => new SecurityException(exception.Exception.Message),
            "DataBaseException" => new DataBaseException(exception.Exception.Message),
            "UserException" => new UserException(exception.Exception.Message),
            "CommunicationException" => new CommunicationException(exception.Exception.Message),
            _ => new Exception(exception.Exception.Message),
        };
