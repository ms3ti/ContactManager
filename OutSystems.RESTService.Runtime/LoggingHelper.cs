...
namespace OutSystems.RESTService {
    public class LoggingHelper {
        ...
        private const string HttpServerVariableServerProtocol = "SERVER_PROTOCOL";
        ...
        public static void SetupLoggingContext(HttpActionContext actionContext, string serviceName, string ssKey) {
            ...
            if (TrySetRESTContextForRESTExposeActions(actionContext, ssKey, ref restExposeContext)
                ...
                HttpService.Context.Items[RESTContextVariableName] = restExposeContext;
            }
        }

        private static void WriteParameterValues(StringBuilder trace, HashSet<string> sensitiveParamsSet, string inputName, string value, bool isBody) {
            if (sensitiveParamsSet != null && sensitiveParamsSet.Contains(inputName)) {
                trace.AppendLine(isBody ? "<redacted>" : $"{inputName} : <redacted>");
            } else {
                trace.AppendLine(value);
            }
        }
        ...
        private static bool TrySetRESTContextForRESTExposeActions(HttpActionContext actionContext, string ssKey, ref RESTExposeContext restExposeContext) {
            ...
            RESTExposeSensitiveInputParameterAttribute sensitiveInputs = actionContext.ActionDescriptor.GetCustomAttributes<RESTExposeSensitiveInputParameterAttribute>().SingleOrDefault();
            RESTExposeSensitiveOutputParameterAttribute sensitiveOutputs = actionContext.ActionDescriptor.GetCustomAttributes<RESTExposeSensitiveOutputParameterAttribute>().SingleOrDefault();
            if (props != null) {
                ...
                if (sensitiveInputs != null) {
                    restExposeContext.SensitiveInputs = sensitiveInputs.SensitiveInputs.ToHashSet();
                }
                if (sensitiveOutputs != null) {
                    restExposeContext.SensitiveOutputs = sensitiveOutputs.SensitiveOutputs.ToHashSet();
                }
        ...
        public static RESTExposeContext GetLoggingContext() {
            return (RESTExposeContext)HttpService.Context.Items[RESTContextVariableName];
        }
        ...
        public static void LogRequest(HttpActionContext actionContext) {
            ...
            if (restExposeContext != null && (restExposeContext.TraceAll || restExposeContext.TraceErrors)) {
                IHttpContext context = HttpService.Context;
                StringBuilder requestTrace = new StringBuilder();
                requestTrace.AppendLine(context.Request.HttpMethod + " " + context.Request.Url.ToString() + " " + context.Request.ServerVariables[HttpServerVariableServerProtocol]);
                WriteRequestHeaders(requestTrace, context, restExposeContext);
                ...
                if (actionContext.Request.Content != null) {
                    ...
                    if (!restExposeContext.IsRequestBinary) {
                        try {
                            ...
                            WriteParameterValues(requestTrace, restExposeContext.SensitiveInputs, "RequestBody", requestEncoding.GetString(content), isBody: true);
                        } catch {
        ...
        public static void LogResponse(HttpActionExecutedContext actionExecutedContext, string errorId = null) {
            ...
            if (restExposeContext != null &&
                ...
                bool hasDetail = actionExecutedContext != null && restExposeContext.RequestTrace != null && (restExposeContext.TraceAll || (restExposeContext.TraceErrors && withError));

                if (restExposeContext.LogTo == LogTo.RESTExpose) {
                    id = IntegrationLog.StaticWrite(appInfo, DateTime.Now, (int)duration.TotalMilliseconds, source, null, restExposeContext.ServiceName + "." + restExposeContext.ActionName, "REST (Expose)", errorIdToLog, true, hasDetail: hasDetail);
                } else {
                    HeContext heContext = appInfo.OsContext;

                    var controller = actionExecutedContext.ActionContext.ControllerContext.Controller as ScreenServicesApiController;
                    string loginId = controller?.MobileSessionInfo?.LoginInfo.LoginId ?? string.Empty;
                    int userId = controller?.MobileSessionInfo?.LoginInfo.UserId ?? 0;
                    string username = controller?.MobileSessionInfo?.LoginInfo.Username ?? string.Empty;
                    ...
                    if (restExposeContext.LogTo == LogTo.ScreenServices) {
                        id = MobileRequestLog.StaticWrite(appInfo, DateTime.Now, restExposeContext.ScreenName, restExposeContext.ActionName, source, (int)duration.TotalMilliseconds, RuntimeEnvironment.MachineName, errorIdToLog, loginId, userId, username, hasDetail: hasDetail);
                        string responseStatus = ((int)actionExecutedContext.Response.StatusCode).ToString() + " " + actionExecutedContext.Response.ReasonPhrase;
                        CookieDebug.Log(context, responseStatus: responseStatus);               
                ...
                if (hasDetail) {
                    ...
                    responseTrace.AppendLine(context.Request.ServerVariables[HttpServerVariableServerProtocol] + " " + ((int)actionExecutedContext.Response.StatusCode).ToString() + " " + actionExecutedContext.Response.ReasonPhrase);
                    WriteResponseHeaders(responseTrace, context, actionExecutedContext, restExposeContext);
                    if (actionExecutedContext.Response != null && actionExecutedContext.Response.Content != null) {
                        if (!restExposeContext.IsResponseBinary) {
                            try {
                                ...
                                Encoding requestEncoding = Encoding.GetEncoding(charset);
                                WriteParameterValues(responseTrace, restExposeContext.SensitiveOutputs, "ResponseBody", requestEncoding.GetString(content), isBody: true);
                            } catch {
        ...
        private static void WriteResponseHeaders(StringBuilder trace, IHttpContext httpContext, HttpActionExecutedContext actionExecutedContext, RESTExposeContext restExposeContext) {
            // HttpContext headers
            var shouldWriteHttpContextHeaders = true;

#if !NETSTANDARD
            shouldWriteHttpContextHeaders = HttpRuntime.UsingIntegratedPipeline;
#endif

            if (shouldWriteHttpContextHeaders) {
                foreach (var headerName in httpContext.Response.Headers.Keys) {
                    foreach (string headerValue in httpContext.Response.Headers.GetValues(headerName)) {
                        WriteParameterValues(trace, restExposeContext.SensitiveOutputs, headerName, $"{headerName}: {headerValue}", isBody: false);
                    }
                }
            }
        ...
        private static void WriteRequestHeaders(StringBuilder trace, IHttpContext httpContext, RESTExposeContext restExposeContext) {
            // HttpContext
            foreach (string headerName in httpContext.Request.Headers.Keys) {
                foreach (string headerValue in httpContext.Request.Headers.GetValues(headerName)) {
                    WriteParameterValues(trace, restExposeContext.SensitiveInputs, headerName, $"{headerName}: {headerValue}", isBody: false);
                }
            }
        }
