	...
	public partial class Actions {
		...
		public static class FuncActionGetApplicationName {
			...
			public static RLApplicationRecordList datasetGetCurrentApplication(HeContext heContext, int maxRecords, IterationMultiplicity multiplicity, out long outParamCount, int qpinESpaceId) {
				...
				myCacheHelper.AddValue(AppUtils.eSpaceId);
				...
				OutSystems.RuntimeCommon.Pair<RLApplicationRecordList, long> temp = RuntimeCache.Instance.Get(new OutSystems.RuntimeCommon.Caching.CacheKey(cacheHash)) as OutSystems.RuntimeCommon.Pair<RLApplicationRecordList, long>;
				if (temp == null) {
					...
					using(var sqlCmd = trans.CreateCommand()) {
						...
						sql = "/* ContactManager.GetApplicationName.GetCurrentApplication */" + sql;
						...
						try {
							...
							if (multiplicity == IterationMultiplicity.Multiple) {
								...
								try {
									...
									RuntimeCache.Instance.Add(new OutSystems.RuntimeCommon.Caching.CacheKey(cacheHash), new OutSystems.RuntimeCommon.Pair<RLApplicationRecordList, long>((RLApplicationRecordList) (outParamList.Duplicate()), -1), new OutSystems.RuntimeCommon.Caching.EspaceTenantDependency(AppUtils.eSpaceId, AppUtils.App.Tenant.Id), DateTime.UtcNow.AddMinutes(60), OutSystems.RuntimeCommon.Caching.CacheUtils.NoSliding, OutSystems.RuntimeCommon.Caching.CacheItemPriority.Removable);
									return outParamList;
								} finally {
