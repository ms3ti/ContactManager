	...
	public class ScrnContact_List: OSPageViewStateCompressed, IWebScreen, INegotiateTabIndexes, IBookmarkableURL {
		...
		public void Preparation(HeContext heContext) {
			...
			try {
				...
				queryResGetContacts_outParamList = FuncssPreparation.datasetGetContacts(heContext, datasetGetContacts_maxRecords, IterationMultiplicity.Single, out queryResGetContacts_outParamCount, Functions.ActionList_SortColumn_GetOrderBy(heContext, wtContactTable.ClientID, "{Contact}.[Name]")
				, ((string) AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"])
				);
		...
		protected void InitializeUrls() {
			...
			if (Request.AppRelativeCurrentExecutionFilePath.IndexOf('/', 2) == -1) {
				...
				actionUrl += AppUtils.GetPageName(heContext, AppUtils.eSpaceId, "Contact_List", parameters,/*useParamsOnlyIfNeededForRule*/ true);
			}
			bookmarkableUrl += AppUtils.GetPageName(heContext, AppUtils.eSpaceId, "Contact_List", parameters,/*useParamsOnlyIfNeededForRule*/ false);
		...
		private void Page_Load(object sender, System.EventArgs e) {
			// init vars
			AppInfo appInfo = AppUtils.App;
			if (appInfo != null) {
				heContext = appInfo.OsContext;
			}
			Response.ContentType = "text/html; charset=" + Response.ContentEncoding.WebName;

			heContext.Session.Validate();
			...
			if (!IsPostBack) {
				ArrayList screenParameters = (ArrayList) AppUtils.App.OsContext.Session["ContactManager._ScreenParameters_Contact_List"];
				...
				if ((screenParametersKey == null || this.Key.Equals(ObjectKey.Parse(Convert.ToString(screenParametersKey)))) && screenParameters != null) {
					try {
						...
						AppUtils.App.OsContext.Session["ContactManager._ScreenParameters_Contact_List"] = null;
		...
		public bool isSecure {
			get {
				return AppUtils.App.IsForcingSecurityForScreens();
			}
		}
		...
		private bool CommandRefreshContactTable(HeContext heContext, bool inParamResetFilters, bool inParamResetPagination) {
			AppUtils.App.Context.Items["osPassedOnAction"] = true;
			CheckPermissions(heContext);
			...
			try {
				...
				try {
					...
					if ((localVars.inParamResetFilters)) {
						...
						AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"] = "";
						explicitChangedVariables.Add(((string) AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"]));
					...
					try {
						...
						AppUtils.App.OsContext.Session["ExceptionMessage"] = ex.Message;
		...
		public string lnk_Link35_NavigateUrl() {
			...
			if ((heContext.AppInfo.eSpaceId != AppUtils.eSpaceId)) {
				navUrl = GetClientRedirectionUrlBasePath(AppUtils.App.IsForcingSecurityForScreens(), AppUtils.Instance.GetImagePath(), "", "");
			} else {
			...
			navUrl += AppUtils.GetPageName(heContext, AppUtils.eSpaceId, "Contact_Edit", parameters);
		...
		public string inputwtSearchInput_input_value() {
			if (("" != ((string) AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"]))) {
				return ((string) AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"]);
			} else {
				...
				if (explicitChangedVariables.Contains(((string) AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"])) || ValidationUtils.ValidateInput(widget, parentEditRecord, ValidationUtils.validateTrue, validate)) {
					return ((string) AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"]);
				} else {
		...
		public void inputwtSearchInput_input_onTextChanged(object sender, EventArgs e) {
			try {
				if ("" == ((OutSystems.HubEdition.WebWidgets.TextBox) sender).Text) {
					...
					AppUtils.App.OsContext.Session["ContactManager.Contact_List_SearchKeyword"] = Convert.ToString(((OutSystems.HubEdition.WebWidgets.TextBox) sender).Text); 
				}
		...
		private void webBlck_WebBlockInstance11_AjaxNotifyTriggered(object sender, EventArgs e) {
			...
			try {
				heContext.CurrentESpaceKey = ssContactManager.AppUtils.eSpaceKey;
		...
		public static class FuncssPreparation {
			...
			/// </summary>
			public static RLContactRecordList datasetGetContacts(HeContext heContext, int maxRecords, IterationMultiplicity multiplicity, out long outParamCount, string qpstList_SortColumn_GetOrderBy, string qpstContact_List_SearchKeyword) {
				...
				// From this point on the difference between Single and the other multiplicities is if it keeps the readers open instead of having the information in memory
				// In some situations we cannot leave readers opened and we need to force the behaviour to be the same as IterationMultiplicity.Multiple
				// {- NOCODE -}
				using(var sqlCmd = trans.CreateCommand()) {
					using(var sqlCountCmd = trans.CreateCommand()) {
						...
						if (qpstContact_List_SearchKeyword != "") {
						...
						sqlCount = "/* ContactManager.MainFlow.Contact_List.Preparation.GetContacts */" + sqlCount;
						sql = "/* ContactManager.MainFlow.Contact_List.Preparation.GetContacts */" + sql;
