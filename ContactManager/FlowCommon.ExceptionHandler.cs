...
namespace ssContactManager.Flows.FlowCommon {
	...
	public class ExceptionHandler {
		...
		public bool HandleException(ref LocalState flowState) {
			...
			if (heContext.Session[SessionInfo.ENTRYPOINT] == null) {
				heContext.Session[SessionInfo.ENTRYPOINT] = HeContext.UnknownEntryPoint;
			}
			while (ex != null) {
				...
				try {
					...
					try {
						if (ex is SecurityException) {
							...
							heContext.Session["ExceptionStack"] = errorStack + ErrorLog.GetStackEnvironmentInfo(AppUtils.App, heContext);
							...
							if (OSPage.IsAjaxRequest) {
								// go to target page
								{
									...
									if (heContext.AppInfo.eSpaceId==AppUtils.eSpaceId) {
										...
										AppUtils.App.OsContext.Session["ContactManager._ScreenParameters_InvalidPermissions"] = screenParameters;
										string sURLQuery = null;
										sURLQuery = (sURLQuery == null ? "": "?" + sURLQuery);
										string sURL = GetClientRedirectionUrlBasePath(AppUtils.App.IsForcingSecurityForScreens(), AppUtils.Instance.GetImagePath(), "", "") + "InvalidPermissions.aspx" + sURLQuery;
							...
							if (OSPage.IsAjaxRequest) {
								// go to target page
								{
									...
									if (heContext.AppInfo.eSpaceId==AppUtils.eSpaceId) {
										...
										ArrayList screenParameters = new ArrayList();
										AppUtils.App.OsContext.Session["ContactManager._ScreenParameters_InternalError"] = screenParameters;
										string sURLQuery = null;
										sURLQuery = (sURLQuery == null ? "": "?" + sURLQuery);
										string sURL = GetClientRedirectionUrlBasePath(AppUtils.App.IsForcingSecurityForScreens(), AppUtils.Instance.GetImagePath(), "", "") + "InternalError.aspx" + sURLQuery;
										DatabaseAccess.FreeupResources(true);
										((OSPage) Page).Redirect(sURL);
									} else {
										string sURLQuery = null;
