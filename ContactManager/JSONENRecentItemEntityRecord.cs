	...
	public class JSONENRecentItemEntityRecord: AbstractRESTStructure<ENRecentItemEntityRecord> {
		...
		public JSONENRecentItemEntityRecord(ENRecentItemEntityRecord s, IBehaviorsConfiguration config) {
			if (config.DefaultValuesBehavior == DefaultValuesBehavior.DontSend) {
				...
				AttrTag = s.ssTag;
				AttrURL = s.ssURL;
				AttrInstant = OutSystems.RESTService.Runtime.Abstractions.Conversions.DateTimeToRestType(s.ssInstant, config.DateTimeFormat);
			}
		}
		...
		public static ENRecentItemEntityRecord ToStructure(ssContactManager.RestRecords.JSONENRecentItemEntityRecord obj, IBehaviorsConfiguration config) {
			...
			if (obj != null) {
				...
				s.ssInstant = obj.AttrInstant == null ? new DateTime(1900, 1, 1, 0, 0, 0): OutSystems.RESTService.Runtime.Abstractions.Conversions.TextToDateTime(obj.AttrInstant, config.DateTimeFormat);
			}
			return s;
		}
