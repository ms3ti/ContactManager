...
namespace ssContactManager {
	public class TimerBootstrap {
		...
		public static string Schedule {
			get {
				using(Transaction trans = DatabaseAccess.ForRuntimeDatabase.GetRequestTransaction()) {
					int eSpaceId = AppUtils.eSpaceId;
		...
		public static int Execute(HeContext heContext, int timeout) {
			...
			var requestTracer = RuntimePlatformUtils.GetRequestTracer();
			if ((requestTracer != null)) {
				requestTracer.MainEventType = RequestTracerEventType.TimerExecuted;
				requestTracer.RegisterAction("9dc6562b-22fb-46da-a6fc-3eff0bfc90a8", "BootstrapContacts");
				requestTracer.RegisterEndpoint(ObjectKeyUtils.DatabaseValue(TimerKey), TimerName);
			}
					...
					using(Transaction privTrans = DatabaseAccess.ForRuntimeDatabase.GetCommitableTransaction()) {
						try {
							...
							if (numberOfTries >= maxNumberOfRetries) {
								...
								errorLogId = ErrorLog.StaticWrite(DateTime.Now, heContext.Session.SessionGuid, heContext.AppInfo.eSpaceId, heContext.AppInfo.Tenant.Id, heContext.Session.UserId, String.Format("Timer Bootstrap reached maximum number of retries ({0}) [Calculating next run]", maxNumberOfRetries), "", "");
	...
	public partial class ExtendedActions {
		...
		public static void WakeBootstrap(HeContext heContext) {
			RuntimePlatformUtils.TestAndSetTimerNextRun(heContext, TimerBootstrap.TimerKey, AppUtils.eSpaceId);
		}
