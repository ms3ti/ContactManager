	...
	public class TimerHandler : System.Web.Services.WebService {
		...
		public int ExecuteTimer(string ssKey, int timeout, int tenantId) {
			...
			if (HttpContext.Current == null) {
				...
				HeContext context = AppUtils.App.OsContext;

				// Check if the request is from a valid IP
				if (!OutSystems.HubEdition.RuntimePlatform.NetworkInterfaceUtils.IsLoopbackAddress(HttpContext.Current.Request.UserHostAddress)) {
...
TimerBootstrap.Execute( context, timeout);
					    ...
					    GeneralLog.StaticWrite(DateTime.Now, context.Session.SessionGuid, AppUtils.eSpaceId,
						    context.AppInfo.Tenant.Id,
                            context.Session.UserId, "Timer '" + ObjectKeyUtils.DatabaseValue(timerKey) + "' isn't mapped",
