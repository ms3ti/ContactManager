...
namespace OutSystems.HubEdition.WebWidgets {
    public abstract class OSPage : BasePage, IObjectWithViewState {
        ...
        public override void DataBind() {
            if (!IsAjaxRequest) {
                ...
                if (RuntimePlatformSettings.Security.EnableSecurityFeaturesISA.GetValue() &&
                    !RuntimePlatformSettings.Security.ApplyCspRulesOnCompileTime.GetValue()
                ) {
                    ...
                    if (appInfo.IsForcingContentSecurityPolicy()) {
                        ...
                        if (!report_To.IsEmpty() && report_To.Contains("<internal>")) {
                            ...
                            var sessionGuid = appInfo.OsContext.Session.SessionGuid;
                            ...
                            try {
                                ...
                                ErrorLog.StaticWrite(DateTime.Now, sessionGuid, espaceID, tenantID, userID, "An exception has occurred while replacing Content Security Policy " +
        ...
        protected void WriteStyleSheetLink(TextWriter writer, string url) {
            var urlPrefix = GetUrlPrefix(out bool prefixWithImagePath);
        ...
        protected virtual void GetExtraStyleSheetIncludes(TextWriter writer) {
            string prefix = GetUrlPrefix(out bool prefixWithImagePath);
        ...
        protected virtual string GetRequestInfoJavaScript(string webScreenKey, string webScreenName) {
            // Since FrontendKey in events is an encrypted of FrontendName this constant is used as a prefix in frontendKey to
            // previne attacks to decrypt any value using Platform private API.
            // RLIT-3562 [The performance probe endpoint provides a way for a user to decrypt values encrypted with the platform private key]
            const string frontendKeyConstant = "QjqLG3H4TTyHLxz";
            ...
            using (var writer = new StringWriter()) {
                ...
                writer.Write("sessionKey:'{0}',", BuiltInFunction.EncodeJavascript((info.OsContext.Session.SessionGuid) ?? ""));
                ...
                if (perfTracerExists) {
                    ...
                    SecureConfidentialInformationEncryption.EncryptWithAlgorithm(frontendKeyConstant + perfTracer.FrontEndName, SecureConfidentialInformationEncryption.FixedKeyAES128.Instance);
                } else {
                    value = string.IsNullOrEmpty(info.FrontendName) ? "" :
                    SecureConfidentialInformationEncryption.EncryptWithAlgorithm(frontendKeyConstant + info.FrontendName, SecureConfidentialInformationEncryption.FixedKeyAES128.Instance);
                }
